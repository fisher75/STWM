...
SESSION_NAME="tracewm_stage2_fullscale_wave1_20260409"
...
echo "[stage2-fullscale-wave1] start: $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "[stage2-fullscale-wave1] python=$PYTHON_BIN"
echo "[stage2-fullscale-wave1] session=$SESSION_NAME"

"$PYTHON_BIN" "$WORK_ROOT/code/stwm/tools/run_tracewm_stage2_fullscale_wave1_20260409.py" \
  --mode orchestrate \
  --work-root "$WORK_ROOT" \
  --python-bin "$PYTHON_BIN" \
  --tmux-session "$SESSION_NAME"

echo "[stage2-fullscale-wave1] done: $(date '+%Y-%m-%d %H:%M:%S %z')"
