...
LOG_PATH="${STWM_ROOT}/logs/tracewm_stage2_semantic_objective_redesign_v3_20260410.log"
mkdir -p "${STWM_ROOT}/logs" "${STWM_ROOT}/tmp"

exec "${PYTHON_BIN}" "${STWM_ROOT}/code/stwm/tools/run_tracewm_stage2_semantic_objective_redesign_v3_20260410.py" --mode all "$@" 2>&1 | tee -a "${LOG_PATH}"
