...
touch "${LOG_PATH}"
exec >> "${LOG_PATH}" 2>&1
echo "[`date -Iseconds`] calibration_only_finalization_pack_run_script_start"
PYTHONUNBUFFERED=1 "${PYTHON_BIN}" "${STWM_ROOT}/code/stwm/tools/run_tracewm_stage2_calibration_only_wave2_20260414.py" --mode all "$@"
