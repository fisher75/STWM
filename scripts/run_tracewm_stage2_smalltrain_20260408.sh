...
LOG_PATH="$WORK_ROOT/logs/tracewm_stage2_smalltrain_${DATE_TAG}.log"
...
CORE_RUN_JSON="$WORK_ROOT/reports/stage2_smalltrain_core_${DATE_TAG}.json"
BURST_RUN_JSON="$WORK_ROOT/reports/stage2_smalltrain_core_plus_burst_${DATE_TAG}.json"
RUNS_JSON="$WORK_ROOT/reports/stage2_smalltrain_runs_${DATE_TAG}.json"
COMPARISON_JSON="$WORK_ROOT/reports/stage2_smalltrain_comparison_${DATE_TAG}.json"
RESULTS_MD="$WORK_ROOT/docs/STAGE2_SMALLTRAIN_RESULTS_${DATE_TAG}.md"

CORE_CKPT_DIR="$WORK_ROOT/outputs/checkpoints/stage2_smalltrain_core_${DATE_TAG}"
BURST_CKPT_DIR="$WORK_ROOT/outputs/checkpoints/stage2_smalltrain_core_plus_burst_${DATE_TAG}"

GPU_SELECT_JSON="$WORK_ROOT/reports/stage2_smalltrain_gpu_selection_${DATE_TAG}.json"
...
echo "[stage2-smalltrain] start: $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "[stage2-smalltrain] python=$PYTHON_BIN"
...
do
  if [[ ! -f "$f" ]]; then
    echo "[stage2-smalltrain] missing_required_file=$f"
    exit 2
  fi
done

echo "[stage2-smalltrain] step=validate_bootstrap_readiness"
"$PYTHON_BIN" - <<PY
import json
...
tao = excluded_map.get("TAO", {})
visor = excluded_map.get("VISOR", {})

if not bool(tao.get("not_in_current_bootstrap", False)):
    raise SystemExit("TAO must remain excluded in this round")
if not bool(visor.get("not_in_current_bootstrap", False)):
    raise SystemExit("VISOR must remain excluded in this round")

print("[stage2-smalltrain] bootstrap_ready=True")
print(f"[stage2-smalltrain] TAO_status={tao.get('status_from_audit', '')}")
print(f"[stage2-smalltrain] VISOR_status={visor.get('status_from_audit', '')}")
PY

echo "[stage2-smalltrain] step=select_gpu_with_selector_and_lease"
GPU_DECISION=$(
  "$PYTHON_BIN" - <<PY
import json
from pathlib import Path
...
lease = acquire_lease(
    ...
    owner='tracewm_stage2_smalltrain_20260408',
    ttl_seconds=8 * 3600,
    lease_path='$GPU_LEASE_PATH',
)
...
if [[ "$SELECTED_GPU" -lt 0 ]]; then
  echo "[stage2-smalltrain] gpu_selection_failed"
  exit 3
fi
...
echo "[stage2-smalltrain] selected_gpu=$SELECTED_GPU lease_id=$LEASE_ID"
echo "[stage2-smalltrain] gpu_metadata=$GPU_METADATA_STR"
...
echo "[stage2-smalltrain] step=run_stage2_smalltrain_core"
"$PYTHON_BIN" "$WORK_ROOT/code/stwm/tracewm_v2_stage2/trainers/train_tracewm_stage2_smalltrain.py" \
  ...
  --auto-resume-latest \
  --run-name stage2_smalltrain_core \
  --run-summary-json "$CORE_RUN_JSON"

echo "[stage2-smalltrain] step=run_stage2_smalltrain_core_plus_burst"
"$PYTHON_BIN" "$WORK_ROOT/code/stwm/tracewm_v2_stage2/trainers/train_tracewm_stage2_smalltrain.py" \
  --stage2-contract-path "$STAGE2_CONTRACT_JSON" \
  --recommended-runtime-json "$RUNTIME_JSON" \
  --use-recommended-runtime \
  ...
  --auto-resume-latest \
  --run-name stage2_smalltrain_core_plus_burst \
  --run-summary-json "$BURST_RUN_JSON"

echo "[stage2-smalltrain] step=summarize_stage2_smalltrain_round"
"$PYTHON_BIN" "$WORK_ROOT/code/stwm/tracewm_v2_stage2/tools/summarize_stage2_smalltrain_round.py" \
...
echo "[stage2-smalltrain] done: $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "[stage2-smalltrain] runs_json=$RUNS_JSON"
echo "[stage2-smalltrain] comparison_json=$COMPARISON_JSON"
echo "[stage2-smalltrain] results_md=$RESULTS_MD"