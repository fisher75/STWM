...
LOG_PATH="$WORK_ROOT/logs/tracewm_stage2_bootstrap_${DATE_TAG}.log"
...
DATA_CONTRACT_JSON="$WORK_ROOT/reports/stage2_bootstrap_data_contract_${DATE_TAG}.json"
DATA_CONTRACT_MD="$WORK_ROOT/docs/STAGE2_BOOTSTRAP_DATA_CONTRACT_${DATE_TAG}.md"
SMOKE_JSON="$WORK_ROOT/reports/stage2_bootstrap_smoke_${DATE_TAG}.json"
RESULTS_MD="$WORK_ROOT/docs/STAGE2_BOOTSTRAP_RESULTS_${DATE_TAG}.md"
VIS_REPORT_JSON="$WORK_ROOT/reports/stage2_bootstrap_visualization_${DATE_TAG}.json"
VIS_IMAGE="$WORK_ROOT/outputs/visualizations/stage2_bootstrap_inputs_${DATE_TAG}.png"
GPU_SELECT_JSON="$WORK_ROOT/reports/stage2_bootstrap_gpu_selection_${DATE_TAG}.json"
...
mkdir -p "$WORK_ROOT/logs" "$WORK_ROOT/reports" "$WORK_ROOT/docs" "$WORK_ROOT/outputs/visualizations"
...
echo "[stage2-bootstrap] step=select_gpu_with_selector_and_lease"
GPU_DECISION=$(
  "$PYTHON_BIN" - <<PY
import json
...
lease = acquire_lease(
    ...
    owner='tracewm_stage2_bootstrap_20260408',
    ttl_seconds=4 * 3600,
    lease_path='$GPU_LEASE_PATH',
)
...
if [[ "$SELECTED_GPU" -lt 0 ]]; then
  echo "[stage2-bootstrap] gpu_selection_failed"
  exit 3
fi
...
echo "[stage2-bootstrap] selected_gpu=$SELECTED_GPU lease_id=$LEASE_ID"
echo "[stage2-bootstrap] gpu_metadata=$GPU_METADATA_STR"
...
echo "[stage2-bootstrap] done: $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "[stage2-bootstrap] data_contract_json=$DATA_CONTRACT_JSON"
echo "[stage2-bootstrap] smoke_json=$SMOKE_JSON"
