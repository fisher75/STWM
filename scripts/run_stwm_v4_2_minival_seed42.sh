...
OUT_ROOT="${1:-$STWM_ROOT/outputs/training/stwm_v4_2_minival_seed42}"
STEPS="${STWM_V4_2_MINIVAL_STEPS:-120}"
SAMPLE_LIMIT="${STWM_V4_2_MINIVAL_SAMPLE_LIMIT:-18}"
SEED="${STWM_V4_2_MINIVAL_SEED:-42}"
...
run_case() {
  ...
  local log_file="$STWM_ROOT/logs/stwm_v4_2_minival_seed42_${run_name}.log"

  echo "[stwm-v4.2-minival] start run=${run_name}"
  PYTHONPATH="$STWM_ROOT/code:${PYTHONPATH:-}" \
    ...
    python "$STWM_ROOT/code/stwm/trainers/train_stwm_v4_2.py" \
      ...
      "$@" \
      >"$log_file" 2>&1
  echo "[stwm-v4.2-minival] done run=${run_name}"
}
...
echo "[stwm-v4.2-minival] all runs done: $OUT_ROOT"
