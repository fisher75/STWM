...
LOG_PATH="$WORK_ROOT/logs/tracewm_stage1_v2_220m_mainline_freeze_${DATE_TAG}.log"
PROTOCOL_DOC="$WORK_ROOT/docs/TRACEWM_STAGE1_V2_220M_MAINLINE_FREEZE_PROTOCOL_${DATE_TAG}.md"
...
MAINLINE_CMP_JSON="$WORK_ROOT/reports/stage1_v2_mainline_confirmation_comparison_${DATE_TAG}.json"
MAINLINE_RUNS_JSON="$WORK_ROOT/reports/stage1_v2_mainline_confirmation_runs_${DATE_TAG}.json"
...
echo "[stage1-v2-freeze] start: $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "[stage1-v2-freeze] python=$PYTHON_BIN"
echo "[stage1-v2-freeze] protocol_doc=$PROTOCOL_DOC"
echo "[stage1-v2-freeze] contract_path=$CONTRACT_PATH"
echo "[stage1-v2-freeze] runtime_json=$RUNTIME_JSON"
echo "[stage1-v2-freeze] minisplit_path=$MINISPLIT_PATH"

if [[ ! -f "$PROTOCOL_DOC" ]]; then
  echo "[stage1-v2-freeze] missing_protocol_doc"
  exit 2
fi

if [[ ! -f "$RUNTIME_JSON" ]]; then
  echo "[stage1-v2-freeze] missing_recommended_runtime_json"
  exit 3
fi

if [[ ! -f "$MINISPLIT_PATH" ]]; then
  echo "[stage1-v2-freeze] missing_stage1_minisplit_json"
  exit 4
fi

if [[ ! -f "$MAINLINE_CMP_JSON" ]]; then
  echo "[stage1-v2-freeze] missing_mainline_confirmation_comparison_json"
  exit 5
fi

if [[ ! -f "$MAINLINE_RUNS_JSON" ]]; then
  echo "[stage1-v2-freeze] missing_mainline_confirmation_runs_json"
  exit 6
fi
...
if [[ "$SELECTED_GPU" -ge 0 ]]; then
  ...
  echo "[stage1-v2-freeze] selected_gpu=$SELECTED_GPU need_mib=$NEED_MIB reason=$GPU_REASON"
else
  echo "[stage1-v2-freeze] gpu_selection_failed need_mib=$NEED_MIB reason=$GPU_REASON"
  exit 7
fi

echo "[stage1-v2-freeze] step=run_mainline_freeze"
"$PYTHON_BIN" "$WORK_ROOT/code/stwm/tracewm_v2/tools/run_stage1_v2_220m_mainline_freeze.py" \
  --contract-path "$CONTRACT_PATH" \
  --recommended-runtime-json "$RUNTIME_JSON" \
  --stage1-minisplit-path "$MINISPLIT_PATH" \
  --mainline-confirmation-comparison-json "$MAINLINE_CMP_JSON" \
  --mainline-confirmation-runs-json "$MAINLINE_RUNS_JSON" \
  --runs-summary-json "$WORK_ROOT/reports/stage1_v2_220m_mainline_freeze_runs_${DATE_TAG}.json" \
  --comparison-json "$WORK_ROOT/reports/stage1_v2_220m_mainline_freeze_comparison_${DATE_TAG}.json" \
  --results-md "$WORK_ROOT/docs/STAGE1_V2_220M_MAINLINE_FREEZE_RESULTS_${DATE_TAG}.md" \
  --epochs 1 \
  --train-steps 192 \
  --eval-steps 16 \
...
echo "[stage1-v2-freeze] done: $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "[stage1-v2-freeze] log=$LOG_PATH"
