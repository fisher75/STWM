...
LOG_PATH="$WORK_ROOT/logs/tracewm_stage1_v2_220m_gap_closure_${DATE_TAG}.log"
PROTOCOL_DOC="$WORK_ROOT/docs/TRACEWM_STAGE1_V2_220M_GAP_CLOSURE_PROTOCOL_${DATE_TAG}.md"
...
echo "[stage1-v2-gap] start: $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "[stage1-v2-gap] python=$PYTHON_BIN"
echo "[stage1-v2-gap] protocol_doc=$PROTOCOL_DOC"
echo "[stage1-v2-gap] contract_path=$CONTRACT_PATH"
echo "[stage1-v2-gap] runtime_json=$RUNTIME_JSON"
echo "[stage1-v2-gap] minisplit_path=$MINISPLIT_PATH"

if [[ ! -f "$PROTOCOL_DOC" ]]; then
  echo "[stage1-v2-gap] missing_protocol_doc"
  exit 2
fi

if [[ ! -f "$RUNTIME_JSON" ]]; then
  echo "[stage1-v2-gap] missing_recommended_runtime_json"
  exit 3
fi

if [[ ! -f "$MINISPLIT_PATH" ]]; then
  echo "[stage1-v2-gap] missing_stage1_minisplit_json"
  exit 4
fi
...
if [[ "$SELECTED_GPU" -ge 0 ]]; then
  ...
  echo "[stage1-v2-gap] selected_gpu=$SELECTED_GPU need_mib=$NEED_MIB reason=$GPU_REASON"
else
  echo "[stage1-v2-gap] gpu_selection_failed need_mib=$NEED_MIB reason=$GPU_REASON"
  exit 5
fi

echo "[stage1-v2-gap] step=run_220m_gap_closure"
"$PYTHON_BIN" "$WORK_ROOT/code/stwm/tracewm_v2/tools/run_stage1_v2_220m_gap_closure.py" \
  --contract-path "$CONTRACT_PATH" \
  --recommended-runtime-json "$RUNTIME_JSON" \
  --stage1-minisplit-path "$MINISPLIT_PATH" \
  --runs-summary-json "$WORK_ROOT/reports/stage1_v2_220m_gap_closure_runs_${DATE_TAG}.json" \
  --comparison-json "$WORK_ROOT/reports/stage1_v2_220m_gap_closure_comparison_${DATE_TAG}.json" \
  --results-md "$WORK_ROOT/docs/STAGE1_V2_220M_GAP_CLOSURE_RESULTS_${DATE_TAG}.md" \
  --epochs 1 \
  --train-steps 40 \
  --eval-steps 8 \
...
echo "[stage1-v2-gap] done: $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "[stage1-v2-gap] log=$LOG_PATH"
