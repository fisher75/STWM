...
OUT_ROOT="${STWM_D1_REPLACEMENT_OUT_ROOT:-$REPO_ROOT/outputs/training/stwm_v4_2_220m_protocol_frozen_frontend_replacement_seed42_v1}"
REPORT_ROOT="${STWM_D1_REPLACEMENT_REPORT_ROOT:-$REPO_ROOT/reports}"

mkdir -p "$OUT_ROOT"
mkdir -p "$REPORT_ROOT"

STAMP="$(date +%Y%m%d_%H%M%S)"
SUBMIT_TSV="${STWM_D1_REPLACEMENT_SUBMIT_TSV:-$REPORT_ROOT/stwm_replacement_clean_matrix_seed42_submit_v1_${STAMP}.tsv}"
...
submit_one() {
  ...
  local notes="Wave1 seed42 replacement clean matrix v1 | data_mode=${DATA_MODE}"
...
submit_one "full_v4_2_seed42_fixed_nowarm_lambda1_rerun_v2" "none" "" "$PREFERRED_GPU_ALL"
submit_one "full_v4_2_seed42_objbias_alpha050_replacement_v1" "none" "0.50" "$PREFERRED_GPU_ALL"
submit_one "wo_semantics_v4_2_seed42_control_v1" "disable_semantics" "" "$PREFERRED_GPU_ALL"
submit_one "wo_object_bias_v4_2_seed42_control_v1" "neutralize_object_bias" "" "$PREFERRED_GPU_ALL"

echo "[replacement-seed42-enqueue] done queue_dir=$QUEUE_DIR"
echo "[replacement-seed42-enqueue] submissions=$SUBMIT_TSV"
