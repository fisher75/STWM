...
# QSTR defaults for this minimal diagnostic matrix.
QSTR_RESIDUAL_SCALE="${STWM_QSTR_RESIDUAL_SCALE:-0.20}"
QSTR_NEUTRAL_PATH_WEIGHT="${STWM_QSTR_NEUTRAL_PATH_WEIGHT:-1.0}"
QSTR_ROUTE_TEMPERATURE="${STWM_QSTR_ROUTE_TEMPERATURE:-0.25}"
QSTR_TEMPORAL_CONSISTENCY_WEIGHT="${STWM_QSTR_TEMPORAL_CONSISTENCY_WEIGHT:-0.10}"

OUT_ROOT="${STWM_QSTR_OUT_ROOT:-$REPO_ROOT/outputs/training/stwm_v4_2_220m_protocol_frozen_frontend_qstr_mainline_seed42_v1}"
REPORT_TSV="${STWM_QSTR_SUBMIT_TSV:-$REPO_ROOT/reports/stwm_qstr_mainline_seed42_submit_v1.tsv}"
REPORT_MD="${STWM_QSTR_SUBMIT_MD:-$REPO_ROOT/docs/STWM_QSTR_MAINLINE_SEED42_SUBMIT_V1.md}"
...
submit_one() {
  ...
  local notes="QSTR seed42 minimal diagnostic matrix v1 | data_mode=${DATA_MODE}"
  local resume_hint="Resume with same output_dir and --auto-resume; compare by official rule within QSTR matrix"
  ...
  # Keep object line frozen as non-headline control for this matrix.
  ...
  case "$mode" in
    ...
    qstr_only)
      cmd+=(
        --qstr-enable
        --qstr-residual-scale "$QSTR_RESIDUAL_SCALE"
        --qstr-neutral-path-weight "$QSTR_NEUTRAL_PATH_WEIGHT"
        --qstr-route-temperature "$QSTR_ROUTE_TEMPERATURE"
      )
      ;;
    qstr_temporal)
      cmd+=(
        ...
        --qstr-neutral-path-weight "$QSTR_NEUTRAL_PATH_WEIGHT"
        --qstr-route-temperature "$QSTR_ROUTE_TEMPERATURE"
        --qstr-temporal-consistency-weight "$QSTR_TEMPORAL_CONSISTENCY_WEIGHT"
      )
      ;;
    *)
...
submit_one "trace_sem_baseline_seed42_qstr_control_v1" "non-object baseline: --neutralize-object-bias" "baseline"
submit_one "qstr_only_seed42_challenge_v1" "qstr-only: query-conditioned semantic residual routing + neutral path" "qstr_only"
submit_one "qstr_temporal_consistency_seed42_challenge_v1" "qstr + temporal semantic consistency regularization" "qstr_temporal"
...
lines = []
lines.append('# STWM QSTR Mainline Seed42 Submit V1')
lines.append('')
...
echo "[qstr-seed42-enqueue] queue_dir=$QUEUE_DIR"
echo "[qstr-seed42-enqueue] submit_tsv=$REPORT_TSV"
echo "[qstr-seed42-enqueue] submit_doc=$REPORT_MD"
