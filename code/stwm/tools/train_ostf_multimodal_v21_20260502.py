...
from stwm.modules.ostf_multimodal_world_model_v21 import OSTFMultimodalConfig, OSTFMultimodalWorldModel
...
def _build_model(kind: str, horizon: int) -> OSTFMultimodalWorldModel:
    num_hyp = 1 if "single_mode" in kind else 6
    cfg = OSTFMultimodalConfig(
        horizon=horizon,
        hidden_dim=384,
        point_dim=192,
        num_layers=4,
        ...
        use_cv_mode=("wo_cv" not in kind and "single_mode" not in kind),
        use_affine_prior=True,
        use_cv_prior=True,
    )
    return OSTFMultimodalWorldModel(cfg)
...
def _sample_weights(records: list[dict[str, Any]]) -> list[float]:
    ...
    for r in records:
        ...
        w = 1.0 + max(0.0, 0.55 * z) + 0.35 * float(r["occlusion_ratio"]) + 0.20 * float(r["reappearance_flag"])
...
def _mode_selection(point_hyp: torch.Tensor, gt_points: torch.Tensor, valid: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
...
def _losses(
    ...
    *,
    ...
    selected, best_idx, ade, end = _mode_selection(point_hyp, fut_points, fut_vis)
    ...
    if use_hard_weighting:
        ...
        sample_weight = 1.0 + torch.clamp(0.70 * hard_z, min=0.0) + 0.35 * occ + 0.20 * reap
        sample_weight = sample_weight.clamp(1.0, 3.5)
    ...
    det_end = F.smooth_l1_loss(point_pred[:, :, -1], fut_points[:, :, -1], reduction="none").sum(dim=-1)
    det_endpoint_per_sample = (det_end * end_mask).sum(dim=1) / end_mask.sum(dim=1).clamp_min(1.0)
    ...
    if use_diversity_loss and point_hyp.shape[3] > 1:
        ...
        if pairwise:
            ...
            diversity_per_sample = F.relu(0.008 - pairwise_dist)
        else:
    ...
    total = (
        ...
        + 0.25 * _weighted_mean_per_sample(extent_per_sample, sample_weight)
        + 0.20 * _weighted_mean_per_sample(vis_per_sample, sample_weight)
        + 0.05 * _weighted_mean_per_sample(sem_ce, sample_weight)
        + 0.10 * _weighted_mean_per_sample(hyp_loss, sample_weight)
        + 0.18 * _weighted_mean_per_sample(det_point_per_sample, sample_weight)
        + 0.10 * _weighted_mean_per_sample(det_endpoint_per_sample, sample_weight)
        + 0.02 * _weighted_mean_per_sample(diversity_per_sample, sample_weight)
...
def _evaluate_model(
    ...
    all_metrics = aggregate_item_rows(rows_mm)
    by_dataset = {ds: aggregate_item_rows(rows_mm, dataset=ds) for ds in sorted({s.dataset for s in samples})}
    subsets = {
        "cv_hard_top20": aggregate_item_rows(rows_mm, subset_key="top20_cv_hard"),
        "occlusion": aggregate_item_rows(rows_mm, subset_key="occlusion_hard"),
        "nonlinear": aggregate_item_rows(rows_mm, subset_key="nonlinear_hard"),
        "interaction": aggregate_item_rows(rows_mm, subset_key="interaction_hard"),
    }
    return all_metrics, by_dataset, subsets, rows_mm, hypothesis_diversity_valid(rows_mm)


def _val_score(all_metrics: dict[str, Any], hard_metrics: dict[str, Any]) -> float:
    return (
        -1.25 * float(hard_metrics["minFDE_K_px"])
        - 0.60 * float(hard_metrics["minADE_K_px"])
        + 18.0 * float(hard_metrics["BestOfK_PCK_16px"])
        + 10.0 * float(hard_metrics["BestOfK_PCK_32px"])
        - 8.0 * float(hard_metrics["MissRate_32px"])
        + 80.0 * float(hard_metrics["weighted_object_extent_iou"])
        - 0.25 * float(all_metrics["weighted_point_L1_px"])
        - 0.15 * float(all_metrics["weighted_endpoint_error_px"])
        + 6.0 * float(all_metrics["BestOfK_PCK_16px"])
        - 2.0 * float(all_metrics["MissRate_32px"])
    )
...
def main() -> int:
    ...
    lr = 6e-4 if "m128" in args.model_kind else 3e-4
    ...
    print(
        f"[V21][start] exp={args.experiment_name} kind={args.model_kind} combo={combo} h={args.horizon} seed={args.seed} steps={args.steps} batch={args.batch_size} device={device}",
        flush=True,
    )
    print(f"[V21][data] train={len(rows['train'])} val={len(rows['val'])} test={len(rows['test'])}", flush=True)
    print(f"[V21][model] params={param_count} init_loaded={len(loaded_keys)} context_cache={context_cache_path}", flush=True)

    use_hard_weighting = True
    use_diversity = "wo_diversity" not in args.model_kind and "single_mode" not in args.model_kind
    best_state = None
    best_val_score = -1e18
    best_step = 0
    loss_history = []
    ckpt_dir = ROOT / "outputs/checkpoints/stwm_ostf_v21"
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    final_rel = f"outputs/checkpoints/stwm_ostf_v21/{args.experiment_name}_final.pt"
    best_rel = f"outputs/checkpoints/stwm_ostf_v21/{args.experiment_name}_best.pt"
    ...
    for step in range(1, args.steps + 1):
        ...
        with autocast(device_type="cuda", enabled=device.type == "cuda"):
            ...
            total, comps = _losses(out, batch, batch_ctx, use_hard_weighting=use_hard_weighting, use_diversity_loss=use_diversity)
        if not torch.isfinite(total):
            print(f"[V21][warn] step={step} nonfinite_total_skip_batch", flush=True)
        ...
        if step == 1 or step % 500 == 0 or step == args.steps:
            ...
            print("[V21][train] " + " ".join(f"{k}={v:.6f}" if isinstance(v, float) else f"{k}={v}" for k, v in row.items()), flush=True)

        if step % args.eval_every == 0 or step == args.steps:
            val_all, _, val_sub, _, val_div = _evaluate_model(model, rows["val"], ctx_map, args.batch_size, device)
            score = _val_score(val_all, val_sub["cv_hard_top20"])
            print(
                f"[V21][val] step={step} weighted_point={val_all['weighted_point_L1_px']:.4f} minfde={val_all['minFDE_K_px']:.4f} "
                f"hard_minfde={val_sub['cv_hard_top20']['minFDE_K_px']:.4f} hard_miss32={val_sub['cv_hard_top20']['MissRate_32px']:.4f} "
                f"hard_bestpck16={val_sub['cv_hard_top20']['BestOfK_PCK_16px']:.4f} div={int(val_div)} score={score:.4f}",
                flush=True,
            )
    ...
    train_all, train_ds, train_sub, train_rows, train_div = _evaluate_model(model, rows["train"], ctx_map, args.batch_size, device)
    val_all, val_ds, val_sub, val_rows, val_div = _evaluate_model(model, rows["val"], ctx_map, args.batch_size, device)
    test_all, test_ds, test_sub, test_rows, test_div = _evaluate_model(model, rows["test"], ctx_map, args.batch_size, device)
    ...
    report = {
        ...
        "metric_note": "Semantic metrics remain secondary in OSTF because future prototype targets are mostly static; multimodal trajectory metrics are primary evidence.",
    }
