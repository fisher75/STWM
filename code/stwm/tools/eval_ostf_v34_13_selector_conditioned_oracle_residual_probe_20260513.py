...
from stwm.tools.train_ostf_v34_13_selector_conditioned_oracle_residual_probe_20260513 import CKPT_DIR, SUMMARY as TRAIN_SUMMARY, compose, model_inputs


SUMMARY = ROOT / "reports/stwm_ostf_v34_13_selector_conditioned_oracle_residual_probe_eval_summary_20260513.json"
DECISION = ROOT / "reports/stwm_ostf_v34_13_selector_conditioned_oracle_residual_probe_decision_20260513.json"
DOC = ROOT / "docs/STWM_OSTF_V34_13_SELECTOR_CONDITIONED_ORACLE_RESIDUAL_PROBE_DECISION_20260513.md"
V3412_DECISION = ROOT / "reports/stwm_ostf_v34_12_local_evidence_oracle_residual_probe_decision_20260513.json"
...
def load_model(args: argparse.Namespace, device: torch.device) -> tuple[SelectorConditionedResidualMemoryV3413 | None, argparse.Namespace | None, dict[str, Any]]:
    ...
    ckpt = ROOT / train.get("checkpoint_path", str(CKPT_DIR / "v34_13_selector_conditioned_oracle_residual_probe_m128_h32_seed42_best.pt"))
    ck = torch.load(ckpt, map_location="cpu")
    ckargs = argparse.Namespace(**ck["args"])
    ckargs.batch_size = args.batch_size
    ckargs.num_workers = args.num_workers
    model = SelectorConditionedResidualMemoryV3413(ckargs.v30_checkpoint, teacher_embedding_dim=ckargs.teacher_embedding_dim, units=ckargs.trace_units, horizon=ckargs.horizon, selector_hidden_dim=ckargs.selector_hidden_dim).to(device)
...
class UnitStats:
    def __init__(self) -> None:
        ...
        self.selector_entropy: list[float] = []
    ...
    def update(self, out: dict[str, torch.Tensor], inst: torch.Tensor, obs_sem: torch.Tensor, obs_mask: torch.Tensor, pos: torch.Tensor) -> None:
        ...
        self.selector_entropy.append(float(out["selector_entropy"].mean().detach().cpu()))
    ...
    def final(self) -> dict[str, float | bool | None]:
        ...
        return {
            ...
            "attention_uses_nontrivial_timesteps": bool(ent is not None and ent < 0.98),
            "selector_entropy": None if not self.selector_entropy else float(np.mean(self.selector_entropy)),
        }
...
def split_eval(split: str, model: SelectorConditionedResidualMemoryV3413, ckargs: argparse.Namespace, device: torch.device) -> dict[str, Any]:
...
def main() -> int:
    ...
    if model is None or ckargs is None:
        decision = {"generated_at_utc": utc_now(), "中文结论": "V34.13 selector-conditioned oracle residual probe 训练未运行，评估跳过。", "oracle_residual_probe_ran": False, "oracle_residual_probe_passed": False, "skip_reason": train.get("skip_reason", "train_not_run"), "recommended_next_step": "fix_nonoracle_measurement_selector"}
        dump_json(SUMMARY, {"generated_at_utc": utc_now(), "decision": decision})
        dump_json(DECISION, decision)
        write_doc(DOC, "V34.13 selector-conditioned oracle residual probe 决策中文报告", decision, ["中文结论", "oracle_residual_probe_ran", "oracle_residual_probe_passed", "skip_reason", "recommended_next_step"])
        print(f"已写出 V34.13 selector-conditioned 评估跳过报告: {DECISION.relative_to(ROOT)}")
        return 0
    ...
    passed = bool(model.v30_backbone_frozen and sem_lb and semantic_signal and nval["stable_preservation"] and ntest["stable_preservation"] and (nval["causal_assignment_subset_gain"] or 0.0) > 0 and (ntest["causal_assignment_subset_gain"] or 0.0) > 0)
    decision = {
        "generated_at_utc": utc_now(),
        "中文结论": "V34.13 selector-conditioned oracle residual probe 已评估；通过前仍不允许 learned gate。",
        ...
        "recommended_next_step": "train_selector_conditioned_residual_gate" if passed else "fix_selector_conditioned_local_evidence_encoder",
    }
    payload = {"generated_at_utc": utc_now(), "train_summary": train, "v34_12_reference": json.loads(V3412_DECISION.read_text(encoding="utf-8")) if V3412_DECISION.exists() else {}, "per_split": per, "decision": decision}
    dump_json(SUMMARY, payload)
    dump_json(DECISION, decision)
    write_doc(DOC, "V34.13 selector-conditioned oracle residual probe 决策中文报告", decision, ["中文结论", "oracle_residual_probe_ran", "oracle_residual_probe_passed", "v30_backbone_frozen", "future_leakage_detected", "trajectory_degraded", "semantic_hard_signal", "changed_semantic_signal", "stable_preservation", "causal_assignment_subset_gain", "strict_residual_subset_gain", "unit_memory_load_bearing_on_residual", "semantic_measurements_load_bearing_on_residual", "assignment_load_bearing_on_residual", "zero_semantic_measurements_metric_delta", "shuffle_semantic_measurements_metric_delta", "selector_ablation_delta", "effective_units", "unit_dominant_instance_purity", "unit_semantic_purity", "recommended_next_step"])
    print(f"已写出 V34.13 selector-conditioned 评估摘要: {SUMMARY.relative_to(ROOT)}")
    return 0
