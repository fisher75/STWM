...
LOG_PATH = ROOT / "logs/stage2_trace_unit_semantic_binding_20260417.log"
DATE_TAG = "20260417"
BOOTSTRAP_BACKEND = "local_clip_vit_b32_mask_crop_visual_teacher"
MAX_CONCURRENT_TUSB_TASKS = 4
ADDITIONAL_TRAIN_STEPS = 1200
BATCH_SIZE = 8
EVAL_INTERVAL = 200
SAVE_EVERY = 200
EVAL_MAX_BATCHES = 0
MAX_TRAIN_PER_DATASET = 1024
MAX_VAL_PER_DATASET = 256
TUSB_ALLOWED_PREFIXES = ("stage2_tusb_lite_",)
...
def _resume_ckpt_for_seed(seed: int) -> Path:
    run_name = f"stage2_calonly_topk1_seed{int(seed)}_wave1_20260413"
    ckpt = ROOT / "outputs/checkpoints" / run_name / "best.pt"
    if not ckpt.exists():
        raise FileNotFoundError(f"missing calibration-only resume checkpoint for seed {seed}: {ckpt}")
    return ckpt
...
def _run_specs() -> List[Dict[str, Any]]:
    ...
    for seed, window_name in [(123, "tusb_s123"), (42, "tusb_s42"), (456, "tusb_s456")]:
        specs.append(
            {
                **common,
                "run_name": f"stage2_tusb_lite_seed{seed}_{DATE_TAG}",
                "seed": int(seed),
                "family": "tusb_lite_main",
                "ablation_name": "main",
                "objective_combo": f"tusb_lite_main_seed{seed}",
                "objective_family": "trace_unit_semantic_binding",
    ...
    specs.append(
        {
            **common,
            "run_name": f"stage2_tusb_lite_no_slowsem_seed123_{DATE_TAG}",
            "seed": 123,
            "family": "tusb_lite_ablation",
            "ablation_name": "no_slowsem",
            "objective_combo": "tusb_lite_no_slowsem_seed123",
            "objective_family": "trace_unit_semantic_binding_ablation",
            "window_name": "tusb_noslow",
            "trace_unit_sem_alpha_min": 1.0,
            "trace_unit_sem_alpha_max": 1.0,
            "trace_unit_semantic_inertia_weight": 0.0,
        }
    )
    specs.append(
        {
            **common,
            "run_name": f"stage2_tusb_lite_no_handshake_seed123_{DATE_TAG}",
            "seed": 123,
            "family": "tusb_lite_ablation",
            "ablation_name": "no_handshake",
            "objective_combo": "tusb_lite_no_handshake_seed123",
            "objective_family": "trace_unit_semantic_binding_ablation",
            "window_name": "tusb_nohand",
            "trace_unit_handshake_layers": 0,
        }
    )
    return specs
...
def _meta_dir(args: Any) -> Path:
    return Path(args.work_root) / "reports/stage2_trace_unit_semantic_binding_runs_20260417"
...
def _count_tusb_params() -> Dict[str, Any]:
    ...
    return {
        ...
        "notes": [
            "parameter count is for newly introduced TUSB-Lite modules only",
...
def _write_frozen_fact_artifacts(args: Any) -> None:
    protocol_payload = {
        ...
        "stage1_frozen_backbone": {
            ...
            "backbone_story": "trace-first future-state backbone remains the most stable component",
        },
        "current_stage2_state": {
            ...
            "current_paper_thickness_level": "cvpr_borderline",
            "future_grounding_usefulness_improved_vs_baselines": False,
        },
        "tusb_lite_goal": {
            "grounding_first_mainline": False,
            "calibration_only_micro_fix": False,
            "target": "bind semantics to frozen trace units so grounding/identifiability becomes a natural readout of semantic trace state",
        },
        "forbidden": [
            "modify_stage1_weights",
            "restore_persistence_mainline",
            "continue_protocol_v4",
            "continue_qualitative_expansion",
    ...
    base._write_md(
        args.protocol_doc,
        [
            "# Stage2 Trace-Unit Semantic Binding Protocol 20260417",
            "",
            ...
            "- current_stage2_interpretation: calibration-only is still the current reasonable mainline, but it is semantic calibration/readout alignment rather than semanticized trace state",
            "- oral_hardening_fact_current_stage2_ready_to_freeze: false",
            "- oral_hardening_fact_current_paper_thickness_level: cvpr_borderline",
            "- oral_hardening_fact_future_grounding_usefulness_improved_vs_baselines: false",
            "- tusb_lite_target: bind semantics to frozen trace units so identifiability is a natural readout of semantic trace state",
            "- forbidden: Stage1 weight changes; persistence revival; protocol v4; qualitative expansion; video/render head; codec/VAE; full architecture search",
        ],
    )
    ...
    instance_payload = {
        "generated_at_utc": now_iso(),
        "main_training_binding_datasets": ["VSPW", "VIPSeg"],
        "protocol_eval_support_datasets": ["VIPSeg", "BURST"],
        "dataset_instance_awareness": {
            "VIPSeg": {
                "true_instance_aware": True,
                "path": "raw panoptic/instance id map preserved into semantic_instance_id_map / crop / temporal",
            },
            "VSPW": {
                "true_instance_aware": False,
                "fallback_mode": "null_or_binary_foreground_only",
                "pseudo_instance_promoted_to_high_quality": False,
            },
            "BURST": {
                "true_instance_aware": True,
                "usage_scope": "protocol v3 / identifiability evaluation only; not returned to main training binding",
            },
        },
        "cache_compatibility": cache_compat,
        "current_binding_note": "main training continues to use approved VSPW + VIPSeg binding, but unit-level supervision now consumes real instance continuity whenever dataset truth exists",
    }
    base._write_json(args.instance_data_report, instance_payload)
    base._write_md(
        args.instance_data_doc,
        [
            "# Stage2 Instance-Aware Data Path 20260417",
            "",
            f"- generated_at_utc: {instance_payload['generated_at_utc']}",
            ...
            f"- predecode_cache_compatible_with_instance_aware_fields: {cache_compat['compatible']}",
            f"- predecode_cache_blocking_reason: {cache_compat['exact_blocking_reason'] or 'none'}",
    ...
    base._write_md(
        ...
        [
            "# Stage2 Trace-Unit Parameter Budget 20260417",
            "",
...
def _tmux_window_command(args: Any, meta_json: Path, meta: Dict[str, Any]) -> str:
    ...
    script_path = Path(args.work_root) / "code/stwm/tools/run_stage2_trace_unit_semantic_binding_20260417.py"
...
def launch(args: Any) -> Dict[str, Any]:
    ...
    payload = {
        ...
        "policy": "TUSB-Lite pilot on frozen Stage1; calibration-only main loss retained; no persistence; max 4 concurrent TUSB training tasks",
...
def summarize(args: Any) -> Dict[str, Any]:
    ...
    if completed_rows:
        main_rows = [row for row in completed_rows if str(row.get("family", "")) == "tusb_lite_main"] or completed_rows
...
def _load_eval_methods(args: Any, best_tusb_run_name: str) -> List[prev_eval.MethodSpec]:
    methods: List[prev_eval.MethodSpec] = [
        ...
        prev_eval.MethodSpec(
            name="no_slowsem_ablation",
            run_name=f"stage2_tusb_lite_no_slowsem_seed123_{DATE_TAG}",
            method_type="stage2",
            checkpoint_path=str(ROOT / "outputs/checkpoints" / f"stage2_tusb_lite_no_slowsem_seed123_{DATE_TAG}" / "best.pt"),
        ),
        prev_eval.MethodSpec(
            name="no_handshake_ablation",
            run_name=f"stage2_tusb_lite_no_handshake_seed123_{DATE_TAG}",
            method_type="stage2",
            checkpoint_path=str(ROOT / "outputs/checkpoints" / f"stage2_tusb_lite_no_handshake_seed123_{DATE_TAG}" / "best.pt"),
        ),
    ]
...
def _evaluate_protocol_v3_for_tusb(args: Any, best_tusb_run_name: str) -> Dict[str, Any]:
    ...
    comparisons = {
        "stage1_frozen_baseline": evalv3._paired_comparison_bundle(per_item, "tusb_lite_best", "stage1_frozen_baseline"),
        "legacysem_best": evalv3._paired_comparison_bundle(per_item, "tusb_lite_best", "legacysem_best"),
        "cropenc_baseline_best": evalv3._paired_comparison_bundle(per_item, "tusb_lite_best", "cropenc_baseline_best"),
        "current_calibration_only_best": evalv3._paired_comparison_bundle(per_item, "tusb_lite_best", "current_calibration_only_best"),
        "no_slowsem_ablation": evalv3._paired_comparison_bundle(per_item, "tusb_lite_best", "no_slowsem_ablation"),
        "no_handshake_ablation": evalv3._paired_comparison_bundle(per_item, "tusb_lite_best", "no_handshake_ablation"),
    }
    by_name = {row["name"]: row for row in method_rows}
    tusb = by_name.get("tusb_lite_best", {})
...
def diagnose(args: Any) -> Dict[str, Any]:
    ...
    if not bool(summary.get("all_runs_terminal", False)) or int(summary.get("failed_count", 0)) > 0 or not completed:
        payload = {
            ...
            "paper_thickness_level": str(oral_diag.get("current_paper_thickness_level", "cvpr_borderline")),
    ...
    main_rows = [row for row in completed if str(row.get("family", "")) == "tusb_lite_main"]
    ...
    tusb_row = method_by_name.get("tusb_lite_best", {})
    cal_row = method_by_name.get("current_calibration_only_best", {})
    no_slowsem_row = method_by_name.get("no_slowsem_ablation", {})
    no_handshake_row = method_by_name.get("no_handshake_ablation", {})

    slowsem_support = bool(
        tusb_row
        and no_slowsem_row
        and float(tusb_row.get("query_future_top1_acc", -1.0)) >= float(no_slowsem_row.get("query_future_top1_acc", -1.0))
        and float(tusb_row.get("hard_subset_top1_acc", -1.0)) >= float(no_slowsem_row.get("hard_subset_top1_acc", -1.0))
    )
    handshake_support = bool(
        tusb_row
        and no_handshake_row
        and float(tusb_row.get("query_future_top1_acc", -1.0)) >= float(no_handshake_row.get("query_future_top1_acc", -1.0))
        and float(tusb_row.get("hard_subset_top1_acc", -1.0)) >= float(no_handshake_row.get("hard_subset_top1_acc", -1.0))
    )
    ...
    if improved_vs_current_calonly and hard_improved and z_sem_slower and assignment_interpretable and slowsem_support and handshake_support:
        next_step = "freeze_tusb_lite_as_new_stage2_mainline"
        paper_level = "cvpr_main_candidate"
    elif improved_vs_current_calonly or hard_improved or z_sem_slower or instance_signal_used:
        next_step = "keep_tusb_lite_direction_but_refine_instance_path"
        paper_level = "stronger_than_cvpr_borderline_but_not_freeze_ready"
    else:
        next_step = "abandon_tusb_lite_and_rethink_stage2_story"
    ...
    payload = {
        ...
        "slowsem_ablation_supports_design": bool(slowsem_support),
        "handshake_ablation_supports_design": bool(handshake_support),
...
def parse_args() -> Any:
    parser = ArgumentParser(description="Run Stage2 Trace-Unit Semantic Binding (TUSB-Lite) pilot")
    ...
    parser.add_argument("--protocol-report", default=str(ROOT / "reports/stage2_trace_unit_semantic_binding_protocol_20260417.json"))
    parser.add_argument("--protocol-doc", default=str(ROOT / "docs/STAGE2_TRACE_UNIT_SEMANTIC_BINDING_PROTOCOL_20260417.md"))
    parser.add_argument("--instance-data-report", default=str(ROOT / "reports/stage2_instance_aware_data_path_20260417.json"))
    parser.add_argument("--instance-data-doc", default=str(ROOT / "docs/STAGE2_INSTANCE_AWARE_DATA_PATH_20260417.md"))
    ...
    parser.add_argument("--launch-report", default=str(ROOT / "reports/stage2_trace_unit_semantic_binding_launch_20260417.json"))
    parser.add_argument("--summary-report", default=str(ROOT / "reports/stage2_trace_unit_semantic_binding_summary_20260417.json"))
    parser.add_argument("--diagnosis-report", default=str(ROOT / "reports/stage2_trace_unit_semantic_binding_diagnosis_20260417.json"))
    parser.add_argument("--results-md", default=str(ROOT / "docs/STAGE2_TRACE_UNIT_SEMANTIC_BINDING_20260417.md"))
