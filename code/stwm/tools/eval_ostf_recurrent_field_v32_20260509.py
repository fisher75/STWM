...
from stwm.tools.train_ostf_recurrent_field_v32_20260509 import build_model, evaluate, make_loader
...
def main() -> int:
    ...
    path = ROOT / f"reports/stwm_ostf_v32_recurrent_field_runs/{args.experiment_name}_eval.json"
    dump_json(path, payload)
    write_doc(
        ROOT / f"docs/STWM_OSTF_V32_RECURRENT_FIELD_EVAL_{args.experiment_name}.md",
        f"STWM OSTF V32 Recurrent Field Eval {args.experiment_name}",
        payload,
        ["checkpoint", "architecture", "field_preserving_rollout", "recurrent_field_dynamics", "test_metrics"],
    )
