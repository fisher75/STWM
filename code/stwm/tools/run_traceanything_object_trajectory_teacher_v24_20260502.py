...
OUT_ROOT = ROOT / "outputs/cache/stwm_traceanything_hardbench_v24"
...
def _select_candidates(
    ...
    *,
    ...
    return _balanced_select(candidates, max_clips)
...
def main() -> int:
    ...
    parser.add_argument("--selection-horizon", type=int, default=64)
    ...
    if not repo.exists() or not ckpt.exists():
        payload = {
            "audit_name": "stwm_traceanything_hardbench_cache_v24_shard",
        ...
        write_doc(doc_path, "STWM TraceAnything Hardbench Cache V24 Shard", payload, list(payload.keys()))
        return 1
    ...
    for idx, meta in enumerate(sharded):
        ...
        try:
            ...
            for m in args.m_values:
                ...
                extract_box = _extract_tracks_for_m_mode(
                    pre, preds, evaluate_bspline_conf, int(query_frame), q_points_raw, object_ids, semantic_ids,
                    raw_size, resized_size, scale_xy, args.obs_len, args.horizon, use_target_box=True
                )
                extract_global = _extract_tracks_for_m_mode(
                    pre, preds, evaluate_bspline_conf, int(query_frame), q_points_raw, object_ids, semantic_ids,
                    raw_size, resized_size, scale_xy, args.obs_len, args.horizon, use_target_box=False
                )
        ...
        print(f"[H{args.horizon} shard {args.shard_index}] {idx+1}/{len(sharded)} ok={len(rows)} failed={len(failures)} {item_key}", flush=True)
