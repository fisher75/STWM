...
def _load_clip_backend(device: str) -> tuple[Any, Any, torch.device, str]:
    ...
    model, preprocess = clip.load("ViT-B/32", device=str(target), jit=False)
    model.eval()
    return clip, model, preprocess, target.type


def parse_args() -> Any:
    p = ArgumentParser(description="Build stronger frozen teacher semantic cache v2 for TUSB-V2")
    p.add_argument("--predecode-cache-root", default=str(ROOT / "data/processed/stage2_tusb_v2_predecode_cache_20260418"))
    p.add_argument("--teacher-cache-root", default=str(ROOT / "data/processed/stage2_teacher_semantic_cache_v2_20260418"))
    p.add_argument("--output-json", default=str(ROOT / "reports/stage2_tusb_teacher_prior_v2_20260418.json"))
    p.add_argument("--output-md", default=str(ROOT / "docs/STAGE2_TUSB_TEACHER_PRIOR_V2_20260418.md"))
...
def main() -> None:
    ...
    clip_mod, clip_model, clip_preprocess, clip_device = _load_clip_backend(str(args.device))
    ...
    feature_dim = 512
    clip_backend = "clip_vit_b32_temporal_masked_mean_v2"
    for key, source_path in sorted(entries.items()):
        ...
        for entity_idx in range(entity_count):
            images = []
            valid_steps = int(temporal_valid[entity_idx].sum()) if entity_idx < temporal_valid.shape[0] else 0
            if valid_steps > 0:
            ...
            with torch.no_grad():
                tensors = torch.stack([clip_preprocess(img) for img in images], dim=0).to(clip_model.dtype if clip_device == "cuda" else torch.float32)
        ...
        np.savez_compressed(
            ...
            backend=np.asarray(clip_backend, dtype=object),
    ...
    payload = {
        ...
        "teacher_backend_v1": "local_clip_vit_b32_mask_crop_visual_teacher",
        "teacher_backend_v2": clip_backend,
        "feature_dim": int(feature_dim),
    ...
    write_md(
        args.output_md,
        [
            "# Stage2 TUSB Teacher Prior V2 20260418",
            "",
            f"- teacher_backend_v1: {payload['teacher_backend_v1']}",
            f"- teacher_backend_v2: {payload['teacher_backend_v2']}",
            ...
            *[f"- {name}: {count}" for name, count in sorted(per_dataset_counts.items())],
            "",
            "## Blocked Alternative Frozen Priors",
            "",
            *[f"- {name}: {reason}" for name, reason in blocked_backends.items()],
        ],
    )
