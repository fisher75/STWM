...
ROOT = prevfix.ROOT
SESSION = "tracewm_stage2_top_tier_hardening_v2_20260416"
LOG_PATH = ROOT / "logs/stage2_top_tier_hardening_v2_20260416.log"
...
def _spec_base(run_name: str, seed: int, ablation_name: str, window_name: str) -> Dict[str, Any]:
    ...
    spec["objective_combo"] = f"hardening_v2_{ablation_name}_seed{int(seed)}"
    spec["track"] = "ablation_fix_v2"
    spec["family"] = "calibration_only_ablation_fix_v2"
    spec["objective_family"] = "calibration_only_mechanism_ablation_fix_v2"
    return spec


def _run_specs() -> List[Dict[str, Any]]:
    specs: List[Dict[str, Any]] = []
    specs.append(_spec_base("stage2_calonly_noalign_seed654_ablate_fix_v2_20260416", 654, "noalign", "abv2_noal_654"))
    specs.append(_spec_base("stage2_calonly_densegate_seed456_ablate_fix_v2_20260416", 456, "densegate", "abv2_dens_456"))
    specs.append(_spec_base("stage2_calonly_densegate_seed654_ablate_fix_v2_20260416", 654, "densegate", "abv2_dens_654"))
    specs.append(_spec_base("stage2_calonly_nodelay_seed456_ablate_fix_v2_20260416", 456, "nodelay", "abv2_node_456"))
    specs.append(_spec_base("stage2_calonly_nodelay_seed654_ablate_fix_v2_20260416", 654, "nodelay", "abv2_node_654"))
    confirm_noalign = _spec_base("stage2_calonly_noalign_seed123_ablate_confirm_20260416", 123, "noalign", "abv2_noal_123c")
    confirm_noalign["confirm_target_run_name"] = "stage2_calonly_noalign_seed123_ablate_fix_20260415"
    confirm_noalign["track"] = "ablation_confirm_v2"
    specs.append(confirm_noalign)
    confirm_nodelay = _spec_base("stage2_calonly_nodelay_seed42_ablate_confirm_20260416", 42, "nodelay", "abv2_node_42c")
    confirm_nodelay["confirm_target_run_name"] = "stage2_calonly_nodelay_seed42_ablate_fix_20260415"
    confirm_nodelay["track"] = "ablation_confirm_v2"
...
def _meta_dir(args: Any) -> Path:
    return Path(args.work_root) / "reports/stage2_mechanism_ablation_fix_v2_runs_20260416"
...
def _build_launch_meta(args: Any, spec: Dict[str, Any], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    meta["track"] = str(spec.get("track", "ablation_fix_v2"))
...
def _tmux_window_command(args: Any, meta_json: Path, meta: Dict[str, Any]) -> str:
    ...
    script_path = Path(args.work_root) / "code/stwm/tools/run_stage2_mechanism_ablation_fix_v2_20260416.py"
    cmd = (
...
def launch(args: Any) -> Dict[str, Any]:
    ...
    payload = {
        ...
        "policy": "real-train only 20260416 targeted hardening v2 ablation fix runs; persistence disabled",
        "actions": actions,
        "runs": runs,
...
def summarize(args: Any) -> Dict[str, Any]:
    ...
    payload = {
        "generated_at_utc": now_iso(),
        "mechanism_ablation_fix_v2_status": f"{running}_running_{completed}_completed_{failed}_failed",
...
def diagnose(args: Any) -> Dict[str, Any]:
    ...
    anomaly_resolution = {
        "stage2_calonly_noalign_seed123_ablate_confirm_20260416": {
        ...
        "stage2_calonly_nodelay_seed42_ablate_confirm_20260416": {
    ...
    prevfix._write_md(
        args.results_md,
        [
            "# Stage2 Mechanism Ablation Fix V2 20260416",
            "",
...
def parse_args() -> Any:
    ...
    parser.add_argument("--prev-mechanism-fix-summary", default=str(ROOT / "reports/stage2_mechanism_ablation_fix_summary_20260415.json"))
    parser.add_argument("--launch-report", default=str(ROOT / "reports/stage2_mechanism_ablation_fix_v2_launch_20260416.json"))
    parser.add_argument("--summary-report", default=str(ROOT / "reports/stage2_mechanism_ablation_fix_v2_summary_20260416.json"))
    parser.add_argument("--diagnosis-report", default=str(ROOT / "reports/stage2_mechanism_ablation_fix_v2_diagnosis_20260416.json"))
    parser.add_argument("--results-md", default=str(ROOT / "docs/STAGE2_MECHANISM_ABLATION_FIX_V2_20260416.md"))
