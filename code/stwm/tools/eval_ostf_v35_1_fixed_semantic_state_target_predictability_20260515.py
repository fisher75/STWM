#!/usr/bin/env python3
"""V35.1 修复版 semantic state targets 的 observed-only predictability 上界审计。"""
...
EVAL_REPORT = ROOT / "reports/stwm_ostf_v35_1_fixed_semantic_state_target_predictability_eval_20260515.json"
DECISION_REPORT = ROOT / "reports/stwm_ostf_v35_1_fixed_semantic_state_target_predictability_decision_20260515.json"
DOC = ROOT / "docs/STWM_OSTF_V35_1_FIXED_SEMANTIC_STATE_TARGET_PREDICTABILITY_DECISION_20260515.md"
FAMILY_NAMES = ["last_visible_evidence", "max_confidence_observed", "instance_pooled_evidence", "changed_transition", "uncertain_abstain"]
...
def build_split(split: str, semantic_clusters: int, max_tokens: int | None, seed: int) -> dict[str, np.ndarray]:
    ...
    split_offset = {"train": 101, "val": 202, "test": 303}.get(split, 404)
...
def main() -> None:
    ...
    args = ap.parse_args()
    print("V35.1: 开始 fixed semantic state target predictability eval，不训练主模型。", flush=True)
    ...
    decision = {
        ...
        "中文结论": "V35.1 只完成修复版 target predictability 上界审计；如果 suite 未 ready，则不训练 semantic state head。",
    }
    eval_report = {
        ...
        "中文结论": "本报告评估可观测输入对 V35.1 fixed semantic state targets 的可预测性，不训练主模型，不进入 V35 head。",
    }
    ...
    print("V35.1 predictability eval 完成。", flush=True)
