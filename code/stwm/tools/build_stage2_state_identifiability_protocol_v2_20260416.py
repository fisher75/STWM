...
def _candidate_tags(item: Dict[str, Any]) -> List[str]:
    return [str(x) for x in item.get("subset_tags", []) if str(x) in PANEL_TARGETS_V2]
...
def _select_candidates_v2(all_candidates: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, List[str]], Dict[str, Any]]:
    ranked = _sorted_candidates(all_candidates)
    panel_members: Dict[str, List[str]] = {key: [] for key in PANEL_TARGETS_V2}
    selected: List[Dict[str, Any]] = []
    selected_ids: set[str] = set()
    panel_candidate_counts = {
        panel: int(sum(panel in _candidate_tags(item) for item in ranked))
        for panel in PANEL_TARGETS_V2
    }
    ...
    for item in ranked:
        ...
        if not tags:
            continue
        useful = [tag for tag in tags if len(panel_members[tag]) < int(PANEL_TARGETS_V2[tag])]
        if not useful:
            continue
        ...
        if (
            ...
            and all(len(panel_members[tag]) >= int(PANEL_TARGETS_V2[tag]) for tag in PANEL_TARGETS_V2)
        ):
            break
    ...
    for panel, target in PANEL_TARGETS_V2.items():
...
def parse_args() -> Any:
    parser = ArgumentParser(description="Build enlarged Stage2 state-identifiability protocol v2")
    parser.add_argument("--contract-json", default=str(ROOT / "reports/stage2_bootstrap_data_contract_20260408.json"))
    parser.add_argument("--output-json", default=str(ROOT / "reports/stage2_state_identifiability_protocol_v2_20260416.json"))
    parser.add_argument("--output-md", default=str(ROOT / "docs/STAGE2_STATE_IDENTIFIABILITY_PROTOCOL_V2_20260416.md"))
...
def main() -> None:
    ...
    selected_items, panel_members, selection_meta = _select_candidates_v2(all_candidates)
    ...
    payload = {
        ...
        "protocol_name": "Stage2 real state-identifiability / future grounding protocol v2",
        ...
        "panel_targets": {k: int(v) for k, v in PANEL_TARGETS_V2.items()},
