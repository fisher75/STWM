...
V3411_CKPT = ROOT / "outputs/checkpoints/stwm_ostf_v34_11_local_semantic_usage_oracle_probe_h32_m128/v34_11_local_semantic_usage_oracle_probe_m128_h32_seed42_best.pt"
V3410_CKPT = ROOT / "outputs/checkpoints/stwm_ostf_v34_10_trace_contract_oracle_residual_probe_h32_m128/v34_10_trace_contract_oracle_residual_probe_m128_h32_seed42_best.pt"
CKPT_DIR = ROOT / "outputs/checkpoints/stwm_ostf_v34_12_local_evidence_oracle_residual_probe_h32_m128"
SUMMARY = ROOT / "reports/stwm_ostf_v34_12_local_evidence_oracle_residual_probe_train_summary_20260513.json"
DOC = ROOT / "docs/STWM_OSTF_V34_12_LOCAL_EVIDENCE_ORACLE_RESIDUAL_PROBE_TRAIN_SUMMARY_20260513.md"
...
def load_compatible(model: MeasurementCausalResidualMemoryV3412) -> dict[str, Any]:
    for name, path in [("v34_11_local_usage", V3411_CKPT), ("v34_10_trace_contract", V3410_CKPT)]:
        if path.exists():
            ...
            return {"init_source": name, "checkpoint_path": str(path.relative_to(ROOT)), "loaded_key_count": len(filt), "missing_key_count": len(missing), "unexpected_key_count": len(unexpected)}
    return {"init_source": "random_new_heads_only"}


def freeze_for_probe(model: MeasurementCausalResidualMemoryV3412) -> None:
...
def model_inputs(batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    return {
        ...
        "teacher_agreement_score": batch["teacher_agreement_score"],
...
def loss_fn(out: dict[str, torch.Tensor], zero: dict[str, torch.Tensor], shuf_sem: dict[str, torch.Tensor], shuf_assign: dict[str, torch.Tensor], batch: dict[str, torch.Tensor], args: argparse.Namespace) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    conf = batch["semantic_measurement_confidence"].float().mean(dim=2).clamp(0.05, 1.0)
    agree = batch["teacher_agreement_score"].float().mean(dim=2).clamp(0.05, 1.0)
    weight = (teacher_w * conf[..., None] * agree[..., None]).clamp(0.02, 1.0)
    sem = cosine_loss(final, batch["fut_teacher_embedding"], pos, weight) if bool(pos.any()) else final.sum() * 0.0
    evidence = cosine_loss(out["local_semantic_evidence_embedding"], batch["fut_teacher_embedding"], pos, weight) if bool(pos.any()) else final.sum() * 0.0
    hard_loss = cosine_loss(final, batch["fut_teacher_embedding"], hard & strict, weight) if bool((hard & strict).any()) else final.sum() * 0.0
    ...
    sem_contrast = masked_mean(F.softplus(args.semantic_contrast_margin - (normal_cos - torch.maximum(zero_cos, shuf_sem_cos))), pos, weight)
    assign_contrast = masked_mean(F.softplus(args.assignment_contrast_margin - (normal_cos - shuf_assign_cos)), pos, weight)
    entropy = masked_mean(out["attention_temporal_entropy"], pos)
    ...
    total = 1.2 * sem + 0.5 * evidence + 0.35 * hard_loss + stable_loss + unit_loss + 0.25 * bind + args.semantic_contrast_weight * sem_contrast + args.assignment_contrast_weight * assign_contrast + args.attention_entropy_weight * entropy + 0.2 * anti
    stats = {
        "loss": float(total.detach().cpu()),
        "final_semantic_target_loss": float(sem.detach().cpu()),
        "local_semantic_evidence_target_loss": float(evidence.detach().cpu()),
        ...
        "attention_entropy_loss": float(entropy.detach().cpu()),
...
def train_one(args: argparse.Namespace) -> dict[str, Any]:
    selector = json.loads(SELECTOR.read_text(encoding="utf-8")) if SELECTOR.exists() else {}
    if not selector.get("nonoracle_measurement_selector_probe_done"):
        payload = {"generated_at_utc": utc_now(), "中文结论": "V34.12 non-oracle selector 未完成，local evidence probe 跳过。", "oracle_residual_probe_ran": False, "skip_reason": "nonoracle_selector_missing"}
        dump_json(SUMMARY, payload)
        write_doc(DOC, "V34.12 local evidence oracle residual probe 训练跳过中文报告", payload, ["中文结论", "oracle_residual_probe_ran", "skip_reason"])
        print(f"已写出 V34.12 local evidence 训练跳过报告: {SUMMARY.relative_to(ROOT)}")
        return payload
    set_seed(args.seed)
    CKPT_DIR.mkdir(parents=True, exist_ok=True)
    device = torch.device("cuda" if torch.cuda.is_available() and not args.cpu else "cpu")
    loader = make_loader("train", args, shuffle=True)
    model = MeasurementCausalResidualMemoryV3412(args.v30_checkpoint, teacher_embedding_dim=args.teacher_embedding_dim, units=args.trace_units, horizon=args.horizon).to(device)
    init = load_compatible(model)
    ...
    for step in range(1, args.steps + 1):
        ...
        if step == 1 or step == args.steps or step % max(100, args.steps // 10) == 0:
            ...
            print(f"训练进度: step={step}/{args.steps}, loss={stats['loss']:.6f}, sem_contrast={stats['semantic_measurement_causal_contrast_loss']:.6f}, attention_entropy={stats['attention_entropy_loss']:.6f}", flush=True)
    ckpt = CKPT_DIR / "v34_12_local_evidence_oracle_residual_probe_m128_h32_seed42_best.pt"
    ...
    payload = {
        ...
        "中文结论": "V34.12 local evidence oracle residual probe 已完成训练；raw temporal semantic measurement 进入 local evidence encoder，未训练 learned gate。",
        ...
        "semantic_measurement_causal_contrast_loss_first": sem_c["first"],
        "semantic_measurement_causal_contrast_loss_last": sem_c["last"],
        "semantic_measurement_causal_contrast_loss_mean": sem_c["mean"],
        "semantic_measurement_causal_contrast_loss_active": sem_c["active"],
        "assignment_contrast_loss_first": ass_c["first"],
        "assignment_contrast_loss_last": ass_c["last"],
        "assignment_contrast_loss_mean": ass_c["mean"],
        "assignment_contrast_loss_active": ass_c["active"],
        "duration_seconds": float(time.time() - start),
        "loss_trace": losses,
    }
    dump_json(SUMMARY, payload)
    write_doc(DOC, "V34.12 local evidence oracle residual probe 训练中文摘要", payload, ["中文结论", "oracle_residual_probe_ran", "fresh_training_completed", "checkpoint_path", "train_sample_count", "v30_backbone_frozen", "future_leakage_detected", "semantic_measurement_causal_contrast_loss_active", "assignment_contrast_loss_active"])
    print(f"已写出 V34.12 local evidence 训练摘要: {SUMMARY.relative_to(ROOT)}")
    return payload
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--attention-entropy-weight", type=float, default=0.04)
