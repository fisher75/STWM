...
OUTPUT_DIR = WORK_ROOT / "outputs" / "visualizations" / "stage1_stage2_qualitative_pack_v2_20260411"
STAGE1_REPORT = WORK_ROOT / "reports" / "stage1_qualitative_pack_v2_20260411.json"
STAGE2_REPORT = WORK_ROOT / "reports" / "stage2_qualitative_pack_v2_20260411.json"
DOC_PATH = WORK_ROOT / "docs" / "STAGE1_STAGE2_QUALITATIVE_PACK_V2_20260411.md"
...
def _wait_for_v6(args: Any) -> Dict[str, Any]:
    deadline = time.time() + float(args.wait_timeout_seconds)
    while time.time() < deadline:
        if Path(args.v6_diagnosis_report).exists():
            payload = _read_json(args.v6_diagnosis_report)
            if bool(payload.get("v6_runs_terminal", False)):
                return payload
        time.sleep(float(args.poll_seconds))
    raise TimeoutError(f"timed out waiting for v6 diagnosis: {args.v6_diagnosis_report}")
...
def _select_stage2_cases(
    ...
    positives_pref = sorted([x for x in candidates if float(x.get("rescue_margin", 0.0)) > 0.005], key=lambda x: (x["rescue_margin"], len(x["subset_tags"])), reverse=True)
    positives_relaxed = sorted([x for x in candidates if float(x.get("rescue_margin", 0.0)) > 0.0], key=lambda x: (x["rescue_margin"], len(x["subset_tags"])), reverse=True)
    failures_pref = sorted([x for x in candidates if float(x.get("failure_margin", 0.0)) > 0.005], key=lambda x: (x["failure_margin"], len(x["subset_tags"])), reverse=True)
    failures_relaxed = sorted([x for x in candidates if float(x.get("failure_margin", 0.0)) > 0.0], key=lambda x: (x["failure_margin"], len(x["subset_tags"])), reverse=True)

    positives: List[Dict[str, Any]] = []
    failures: List[Dict[str, Any]] = []
    _extend_unique(positives, positives_pref, limit=4)
    _extend_unique(positives, positives_relaxed, limit=4)
    _extend_unique(failures, failures_pref, limit=4)
    _extend_unique(failures, failures_relaxed, limit=4)
...
def build_stage1_pack(args: Any, core_ds: Any, stage1_model: Any, device: torch.device) -> Dict[str, Any]:
    ...
    payload = {
        ...
        "pack_type": "stage1_qualitative_pack_v2",
...
def build_stage2_pack(args: Any, core_ds: Any, stage1_model: Any, device: torch.device, v6_diag: Dict[str, Any]) -> Dict[str, Any]:
    manifest = _read_json(args.semantic_hard_manifest_path)
    rescue_run = str(v6_diag.get("success_criteria", {}).get("semantic_hard_best_run_name", "") or v6_diag.get("success_criteria", {}).get("overall_best_run_name", ""))
    if not rescue_run:
        raise RuntimeError("v6 diagnosis missing best rescue run")
    ...
    payload = {
        "generated_at_utc": now_iso(),
        "pack_type": "stage2_qualitative_pack_v2",
        "selection_policy": {
...
def write_doc(path: str | Path, stage1_pack: Dict[str, Any], stage2_pack: Dict[str, Any]) -> None:
    lines = [
        "# Stage1 / Stage2 Qualitative Pack V2",
        "",
...
def parse_args() -> Any:
    p = ArgumentParser(description="Build Stage1/Stage2 qualitative packs v2 for manual inspection")
...
def main() -> None:
    ...
    eval_gpu = _acquire_eval_gpu(args, owner="stage1_qualitative_pack_v2_20260411")
    try:
        import os
    ...
    v6_diag = _wait_for_v6(args)

    eval_gpu = _acquire_eval_gpu(args, owner="stage2_qualitative_pack_v2_20260411")
    try:
        import os
        ...
        core_ds = _make_dataset(["vspw", "vipseg"], "val", str(args.stage2_contract_json), max_samples=-1)
        stage1_model, _ = _load_stage1_model(device=device, stage1_ckpt=str(args.stage1_best_ckpt))
        stage2_pack = build_stage2_pack(args, core_ds, stage1_model, device, v6_diag)
