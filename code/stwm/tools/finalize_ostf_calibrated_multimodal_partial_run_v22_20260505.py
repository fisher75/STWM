...
from stwm.tools.train_ostf_calibrated_multimodal_v22_20260502 import build_model, evaluate_model, prepare_rows_for_model


TRAIN_RE = re.compile(r"\[V22\]\[train\]\s+(.*)")
VAL_RE = re.compile(r"\[V22\]\[val\]\s+(.*)")
...
def main() -> int:
    ...
    train_all, train_ds, train_sub, train_rows_mm, train_cal = evaluate_model(model, rows["train"], ctx_map, args.batch_size, device)
    val_all, val_ds, val_sub, val_rows_mm, val_cal = evaluate_model(model, rows["val"], ctx_map, args.batch_size, device)
    test_all, test_ds, test_sub, test_rows_mm, test_cal = evaluate_model(model, rows["test"], ctx_map, args.batch_size, device)
    ...
    report = {
        ...
        "metric_note": "V22 run finalized from current best checkpoint after real partial training. This preserves real trajectory evidence without claiming a completed 30k schedule.",
    }
