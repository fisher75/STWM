...
def _stage2_case_from_item_v9(case_id: str, tags: List[str], why: str, interpretation: str, item: Dict[str, Any]) -> Dict[str, Any]:
...
def main() -> None:
    ...
    parser.add_argument("--stage1-prev", default=str(ROOT / "reports/stage1_qualitative_pack_v8_20260416.json"))
    parser.add_argument("--state-ident-eval-v3", default=str(ROOT / "reports/stage2_state_identifiability_eval_v3_20260416.json"))
    parser.add_argument("--mechanism-fix-v3-summary", default=str(ROOT / "reports/stage2_mechanism_ablation_fix_v3_summary_20260416.json"))
    ...
    parser.add_argument("--local-temporal-summary", default=str(ROOT / "reports/stage2_local_temporal_semantic_branch_summary_20260416.json"))
    parser.add_argument("--stage1-output", default=str(ROOT / "reports/stage1_qualitative_pack_v9_20260416.json"))
    parser.add_argument("--stage2-output", default=str(ROOT / "reports/stage2_qualitative_pack_v9_20260416.json"))
    parser.add_argument("--output-md", default=str(ROOT / "docs/STAGE1_STAGE2_QUALITATIVE_PACK_V9_20260416.md"))
    args = parser.parse_args()

    stage1_prev = prev.read_json(args.stage1_prev)
    state_eval = prev.read_json(args.state_ident_eval_v3)
    mechanism_summary = prev.read_json(args.mechanism_fix_v3_summary)
    mechanism_diag = prev.read_json(args.mechanism_fix_v3_diagnosis)
    local_temporal_summary = prev.read_json(args.local_temporal_summary)
    ...
    if cal_win:
            cases.append(_stage2_case_from_item_v9("stage2_calibration_clear_win_v9", ["calibration-only", "state-identifiability-success"], "Calibration-only clear win on protocol v3.", "Primary Stage2 success figure candidate.", cal_win))
    if cal_fail:
            cases.append(_stage2_case_from_item_v9("stage2_calibration_fail_v9", ["calibration-only-fail", "state-identifiability-failure"], "Calibration-only failure case retained for balance.", "Use to bound claims and highlight remaining hard regimes.", cal_fail))
    if legacy_win:
        cases.append(_stage2_case_from_item_v9("stage2_legacysem_win_v9", ["legacysem-win"], "Legacysem wins on this item.", "Residual failure regime.", legacy_win))
    if crop_win:
        cases.append(_stage2_case_from_item_v9("stage2_cropenc_win_v9", ["cropenc-win"], "Cropenc wins on this item.", "Residual failure regime.", crop_win))
    if noalign_fail:
        cases.append(_stage2_case_from_item_v9("stage2_noalign_failure_v9", ["noalign-failure"], "Noalign ablation failure representative.", "Supports alignment being load-bearing when mechanism evidence closes.", noalign_fail))
    if dense_fail:
        cases.append(_stage2_case_from_item_v9("stage2_densegate_failure_v9", ["densegate-failure"], "Densegate ablation failure representative.", "Supports sparse gating selectivity.", dense_fail))
    if nodelay_fail:
        cases.append(_stage2_case_from_item_v9("stage2_nodelay_failure_v9", ["nodelay-failure"], "Nodelay ablation failure representative.", "Supports delayed auxiliary schedule.", nodelay_fail))
    if anomaly_info:
        anomaly_case = cal_fail or cal_win
        if anomaly_case:
            cases.append(_stage2_case_from_item_v9("stage2_anomaly_scope_v9", ["anomaly-check"], "Anomaly confirmation/rejection context case.", f"Anomaly status: {json.dumps(anomaly_info, ensure_ascii=True)}", anomaly_case))
    if cal_win:
        cases.append(_stage2_case_from_item_v9("stage2_state_identifiability_success_v9", ["state-identifiability-success"], "Protocol v3 future grounding success case.", "Direct protocol contribution figure candidate.", cal_win))
    if cal_fail:
        cases.append(_stage2_case_from_item_v9("stage2_state_identifiability_failure_v9", ["state-identifiability-failure"], "Protocol v3 failure case.", "Hard negative for oral backup and paper balance.", cal_fail))
    local_cases = local_temporal_summary.get("case_candidates", []) if isinstance(local_temporal_summary.get("case_candidates", []), list) else []
    if local_cases:
        exemplar = local_cases[0] if isinstance(local_cases[0], dict) else {}
        anchor = cal_win or cal_fail or legacy_win or crop_win
        if anchor:
            cases.append(
                _stage2_case_from_item_v9(
                    "stage2_local_temporal_branch_v9",
                    ["local-temporal-branch"],
                    "Local temporal semantic branch gain / no-gain case.",
                    f"Local temporal pilot summary: {json.dumps(exemplar, ensure_ascii=True)}",
                    anchor,
                )
            )
    ...
    stage2_payload = {
        ...
        "state_identifiability_eval_source": str(args.state_ident_eval_v3),
        "mechanism_fix_v3_summary_source": str(args.mechanism_fix_v3_summary),
        "local_temporal_summary_source": str(args.local_temporal_summary),
        "cases": cases[:18],
    }
