...
LOG_PATH = WORK_ROOT / "logs/tracewm_stage2_semantic_objective_redesign_v4_20260411.log"
...
def _run_specs() -> List[Dict[str, Any]]:
    return [
        {
            "run_name": "stage2_semobjv4_quantile_align_seed42_20260411",
            "seed": 42,
            "objective_combo": "quantile_sparse_gating+confidence_gated_readout_alignment",
            "semantic_rescue_mode": "v4sparse",
            ...
            "v4_sparse_gating_family": "quantile_sparse_gating",
            "v4_gating_quantile": 0.90,
            "v4_topk_token_ratio": 0.20,
            "v4_topk_min_tokens": 2,
            "v4_gate_min_strength": 0.08,
            "v4_persistence_value_quantile": 0.75,
            "v4_persistence_max_pairs": 48,
            "v4_persistence_margin": 0.10,
            "aux_loss_delay_steps": 0,
            "aux_loss_ramp_steps": 0,
            "semantic_hard_sidecar_enabled": True,
            "window_name": "semobjv4_qa42",
        },
        {
            "run_name": "stage2_semobjv4_topk_align_seed42_20260411",
            "seed": 42,
            "objective_combo": "topk_query_gating+confidence_gated_readout_alignment",
            "semantic_rescue_mode": "v4sparse",
            "semantic_rescue_weight": 0.0002,
            "confidence_gated_alignment_loss_weight": 1.0,
            "sparse_persistence_contrastive_loss_weight": 0.0,
            ...
            "confidence_gating_temperature": 0.05,
            "semantic_hard_score_threshold": 0.25,
            "v4_sparse_gating_family": "topk_query_gating",
            "v4_gating_quantile": 0.90,
            "v4_topk_token_ratio": 0.15,
            "v4_topk_min_tokens": 2,
            "v4_gate_min_strength": 0.08,
            "v4_persistence_value_quantile": 0.75,
            "v4_persistence_max_pairs": 48,
            ...
            "aux_loss_ramp_steps": 0,
            "semantic_hard_sidecar_enabled": True,
            "window_name": "semobjv4_ta42",
        },
        {
            "run_name": "stage2_semobjv4_quantile_persistdelay_seed42_20260411",
            "seed": 42,
            "objective_combo": "quantile_sparse_gating+high_value_sparse_persistence+strong_delayed_aux_schedule",
            "semantic_rescue_mode": "v4sparse",
            "semantic_rescue_weight": 0.0002,
            "confidence_gated_alignment_loss_weight": 1.0,
            ...
            "v4_persistence_max_pairs": 48,
            "v4_persistence_margin": 0.10,
            "aux_loss_delay_steps": 120,
            "aux_loss_ramp_steps": 240,
            "semantic_hard_sidecar_enabled": True,
            "window_name": "semobjv4_qpd42",
        },
        {
            "run_name": "stage2_semobjv4_quantile_persistdelay_seed123_20260411",
            "seed": 123,
            "objective_combo": "quantile_sparse_gating+high_value_sparse_persistence+strong_delayed_aux_schedule",
            "semantic_rescue_mode": "v4sparse",
            "semantic_rescue_weight": 0.0002,
            ...
            "aux_loss_ramp_steps": 240,
            "semantic_hard_sidecar_enabled": True,
            "window_name": "semobjv4_qpd123",
        },
        {
            "run_name": "stage2_semobjv4_topk_persistdelay_seed42_20260411",
            "seed": 42,
            "objective_combo": "topk_query_gating+high_value_sparse_persistence+strong_delayed_aux_schedule",
            "semantic_rescue_mode": "v4sparse",
            "semantic_rescue_weight": 0.0002,
            "confidence_gated_alignment_loss_weight": 1.0,
            "sparse_persistence_contrastive_loss_weight": 0.05,
            ...
            "aux_loss_ramp_steps": 240,
            "semantic_hard_sidecar_enabled": True,
            "window_name": "semobjv4_tpd42",
        },
        {
            "run_name": "stage2_semobjv4_topk_persistdelay_seed123_20260411",
            "seed": 123,
            "objective_combo": "topk_query_gating+high_value_sparse_persistence+strong_delayed_aux_schedule",
            "semantic_rescue_mode": "v4sparse",
            "semantic_rescue_weight": 0.0002,
...
def launch(args: Any) -> Dict[str, Any]:
    ...
    for spec in _run_specs():
        ...
        meta_json = Path(args.work_root) / "reports" / "stage2_semantic_objective_redesign_v4_runs_20260411" / f"{run_name}_launch_meta.json"
        ...
        cmd = (
            ...
            f"{shlex.quote(str(Path(args.work_root) / 'code/stwm/tools/run_tracewm_stage2_semantic_objective_redesign_v4_20260411.py'))} "
    ...
    payload = {
        ...
        "mode": "stage2_semantic_objective_redesign_v4_launch",
    ...
    _write_json(args.v4_launch_report, payload)
...
def run_one(args: Any) -> None:
    ...
    cmd = [
        ...
        "--v4-sparse-gating-family", str(meta["v4_sparse_gating_family"]),
        "--v4-gating-quantile", str(meta["v4_gating_quantile"]),
        "--v4-topk-token-ratio", str(meta["v4_topk_token_ratio"]),
        "--v4-topk-min-tokens", str(meta["v4_topk_min_tokens"]),
        "--v4-gate-min-strength", str(meta["v4_gate_min_strength"]),
        "--v4-persistence-value-quantile", str(meta["v4_persistence_value_quantile"]),
        "--v4-persistence-max-pairs", str(meta["v4_persistence_max_pairs"]),
        "--v4-persistence-margin", str(meta["v4_persistence_margin"]),
...
def summarize(args: Any) -> Dict[str, Any]:
    launch = _read_json(args.v4_launch_report)
    ...
    for meta in launch.get("runs", []) if isinstance(launch.get("runs", []), list) else []:
        ...
        rows.append({
            ...
            "v4_sparse_gating_family": str(meta.get("v4_sparse_gating_family", "")),
            ...
            "sparse_gate_selected_ratio_mean": float(branch.get("sparse_gate_selected_ratio_mean", 0.0)),
            "high_value_pair_ratio_mean": float(branch.get("high_value_pair_ratio_mean", 0.0)),
    ...
    payload = {
        "generated_at_utc": now_iso(),
        "redesign_v4_status": f"{running}_running_{completed}_completed_{failed}_failed",
        "runs": rows,
        "next_step_choice_internal": "summarize_redesign_v4_after_completion" if completed == len(rows) and rows and failed == 0 else ("fix_failed_v4_runs" if failed else "continue_redesign_v4"),
    }
    _write_json(args.v4_summary_report, payload)
    lines = [
        "# Stage2 Semantic Objective Redesign V4 Results",
        "",
        f"- generated_at_utc: {payload['generated_at_utc']}",
        f"- redesign_v4_status: {payload['redesign_v4_status']}",
        "",
    ...
    for row in rows:
        ...
        lines.append(
            f"| {row['run_name']} | {row['objective_combo']} | {row['v4_sparse_gating_family']} | {row['selected_gpu_id']} | {row['batch_size']} | {row['train_steps']} | {row['status']} | {_f(best.get('free_rollout_endpoint_l2'), 1e9):.8f} | {float(row.get('semantic_hard_sidecar_score', 1e9)):.8f} | {float(row.get('confidence_gated_affected_sample_ratio_mean', 0.0)):.4f} | {float(row.get('sparse_gate_selected_ratio_mean', 0.0)):.4f} | {float(row.get('high_value_pair_ratio_mean', 0.0)):.4f} |"
        )
    _write_md(args.v4_results_md, lines)
    return payload
...
def wait_for_completion(args: Any) -> Dict[str, Any]:
    ...
    while time.time() < deadline:
        if str(last.get("redesign_v4_status", "")).startswith("0_running_"):
            return last
        time.sleep(float(args.poll_seconds))
        last = summarize(args)
    last["timed_out_waiting_for_completion"] = True
    _write_json(args.v4_summary_report, last)
    return last


def diagnose_v4(args: Any) -> Dict[str, Any]:
    summary = _read_json(args.v4_summary_report)
    ...
    payload: Dict[str, Any] = {
        "generated_at_utc": now_iso(),
        "diagnosis_type": "stage2_semantic_objective_redesign_v4",
        "teacher_as_mainline_semantic_source": False,
        "chosen_bootstrap_backend": "local_clip_vit_b32_mask_crop_visual_teacher",
        "v4_runs_terminal": bool(rows and len(completed) == len(rows)),
        "baseline_refs": {
    ...
    if not payload["v4_runs_terminal"]:
        payload["success_criteria"] = {
            "true_new_best_not_warm_start_inherited": False,
            "gating_not_saturated_anymore": False,
            "high_value_pair_filter_active": False,
            "semantic_hard_composite_improved_vs_v3": False,
            ...
            "improved_vs_v3_best_objective_combo": False,
            "best_v4_objective_combo": "none",
            "best_v4_full_validation_endpoint_l2": 1e9,
            "best_v4_semantic_hard_composite_score": 1e9,
            "overall_best_and_semantic_hard_best_diverged": False,
            "next_step_choice": "continue_redesign_v4",
        }
        _write_json(args.v4_diagnosis_report, payload)
        return payload
    ...
    try:
        print(f"[semobjv4] diagnosis device={device} eval_gpu={eval_gpu}", flush=True)
        ...
        for row in completed:
            ...
            print(f"[semobjv4] full validation eval run={run_name}", flush=True)
            ...
            run_aux[run_name] = {
                ...
                "sparse_gate_selected_ratio": float(branch.get("sparse_gate_selected_ratio_mean", 0.0)),
                "high_value_pair_ratio": float(branch.get("high_value_pair_ratio_mean", 0.0)),
                "semantic_hard_sidecar_metric": sidecar,
        ...
        for subset_name in ["occlusion_reappearance", "crossing_or_interaction_ambiguity", "small_object_or_low_area", "appearance_change_or_semantic_shift"]:
            print(f"[semobjv4] semantic hard eval subset={subset_name}", flush=True)
            ...
            "runs": [
                {
                    ...
                    "sparse_gate_selected_ratio": float(run_aux.get(run_name, {}).get("sparse_gate_selected_ratio", 0.0)),
                    "high_value_pair_ratio": float(run_aux.get(run_name, {}).get("high_value_pair_ratio", 0.0)),
                }
        ...
        try:
            print("[semobjv4] optional BURST persistence-hard eval", flush=True)
    ...
    gating_not_saturated = any(float(run_aux.get(str(row["run_name"]), {}).get("sparse_gate_selected_ratio", 1.0)) < 0.90 for row in completed)
    high_value_pair_active = any(float(run_aux.get(str(row["run_name"]), {}).get("high_value_pair_ratio", 0.0)) > 0.05 for row in completed)
    hard_improved_vs_v3 = bool(best_hard_composite < float(v3_best_hard_comp))
    hard_improved_vs_v2 = bool(best_hard_composite < float(v2_best_hard_comp))
    non_catastrophic = bool(best_full_endpoint <= crop_ep * 1.5)
    improved_vs_cropenc = bool(best_full_endpoint < crop_ep)
    improved_vs_v3 = bool(best_hard_composite < float(v3_best_hard_comp))
    ...
    if len({int(x.get("seed", -1)) for x in combo_rows}) >= 2:
        ...
        cross_seed_support = bool(min(vals) <= float(v3_best_hard_comp) and max(vals) <= float(v3_best_hard_comp) * 1.20)
    ...
    if true_new_best and gating_not_saturated and high_value_pair_active and hard_improved_vs_v3 and cross_seed_support and non_catastrophic:
        next_step = "stage2_semantic_rescue_fullscale_wave1"
    elif improved_vs_v3 or hard_improved_vs_v2 or overall_semantic_hard_diverged:
        next_step = "freeze_stage2_core_mainline"
    else:
    ...
    payload["success_criteria"] = {
        ...
        "gating_not_saturated_anymore": bool(gating_not_saturated),
        "high_value_pair_filter_active": bool(high_value_pair_active),
        "semantic_hard_composite_improved_vs_v3": bool(hard_improved_vs_v3),
        ...
        "improved_vs_v3_best_objective_combo": bool(improved_vs_v3),
        "best_v4_objective_combo": str(best_combo),
        "best_v4_full_validation_endpoint_l2": float(best_full_endpoint),
        "best_v4_semantic_hard_composite_score": float(best_hard_composite),
...
def run_all(args: Any) -> Dict[str, Any]:
    ...
    diag = diagnose_v4(args)
    return {"v4_diagnosis": diag}


def parse_args() -> Any:
    p = ArgumentParser(description="Stage2 semantic objective redesign v4")
    ...
    p.add_argument("--v4-protocol-doc", default=str(WORK_ROOT / "docs/STAGE2_SEMANTIC_OBJECTIVE_REDESIGN_V4_PROTOCOL_20260411.md"))
    p.add_argument("--v4-launch-report", default=str(WORK_ROOT / "reports/stage2_semantic_objective_redesign_v4_launch_20260411.json"))
    p.add_argument("--v4-summary-report", default=str(WORK_ROOT / "reports/stage2_semantic_objective_redesign_v4_summary_20260411.json"))
    p.add_argument("--v4-results-md", default=str(WORK_ROOT / "docs/STAGE2_SEMANTIC_OBJECTIVE_REDESIGN_V4_RESULTS_20260411.md"))
    p.add_argument("--v4-diagnosis-report", default=str(WORK_ROOT / "reports/stage2_semantic_objective_redesign_v4_diagnosis_20260411.json"))
...
def main() -> None:
    ...
    if args.mode == "all":
        ...
        print(json.dumps(diagnose_v4(args), ensure_ascii=True, indent=2))
