...
OUT_DIR = ROOT / "assets/videos/stwm_ostf_v19"
REPORT_PATH = ROOT / "reports/stwm_ostf_v19_visualization_20260502.json"
TEACHER_COLOR = (255, 255, 255)
CV_COLOR = (255, 196, 0)
V18_COLOR = (80, 220, 255)
V19_COLOR = (46, 204, 113)
...
def _render_case(case_index: int, sample: OSTFObjectSample, cv_pred: np.ndarray, cv_vis: np.ndarray, v18_pred: np.ndarray, v18_vis: np.ndarray, v19_pred: np.ndarray, v19_vis: np.ndarray) -> dict[str, Any]:
    ...
    v18_tr = _transform(np.concatenate([sample.obs_points, v18_pred], axis=1) * scale_raw, scale, ox, oy)
    ...
    for t, frame_path in enumerate(frame_paths[:total_t]):
        ...
        draw.text((16, 8), f"OSTF V19 case={case_index:02d} dataset={sample.dataset} item={sample.item_key} obj={sample.object_id} t={t}", fill=(255, 255, 255))
        draw.text((16, 30), "white=teacher  yellow=CV  cyan=V18  green=V19", fill=(255, 220, 160))
...
def main() -> int:
    ...
    v19_report = load_json(ROOT / "reports/stwm_ostf_v19_runs/v19_refinement_m512_seed42_h8.json")
    ...
    cases = []
    for case_index, key in enumerate(_choose_cases(v19_report["item_scores"], count=20)):
        ...
        cases.append(_render_case(case_index, sample, cv_pred, cv_vis, v18_pred, v18_vis, v19_pred, v19_vis))
    payload = {
        "audit_name": "stwm_ostf_v19_visualization",
        ...
        "cases": cases,
        "teacher_source": "cotracker_official",
        "note": "Each GIF shows raw temporal frames, teacher dense tracks, constant-velocity prior, V18 prediction, and V19 refinement prediction.",
    }
