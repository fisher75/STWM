...
SUMMARY = ROOT / "reports/stwm_ostf_v34_9_trace_fixed_residual_gate_train_summary_20260512.json"
DOC = ROOT / "docs/STWM_OSTF_V34_9_TRACE_FIXED_RESIDUAL_GATE_TRAIN_SUMMARY_20260512.md"
...
def main() -> int:
    ...
    if not oracle.get("oracle_residual_probe_passed"):
        payload = {
            ...
            "中文结论": "V34.9 oracle residual probe 未通过，按协议不训练 learned gate。",
        ...
        write_doc(DOC, "V34.9 trace-fixed residual gate 训练中文摘要", payload, ["中文结论", "learned_gate_training_ran", "learned_gate_passed", "skip_reason"])
        print(f"已写出 V34.9 gate 训练跳过报告: {SUMMARY.relative_to(ROOT)}")
        return 0
    ...
    dump_json(SUMMARY, payload)
    write_doc(DOC, "V34.9 trace-fixed residual gate 训练中文摘要", payload, ["中文结论", "learned_gate_training_ran", "learned_gate_passed", "skip_reason"])
    print(f"已写出 V34.9 gate 训练报告: {SUMMARY.relative_to(ROOT)}")
    return 0
