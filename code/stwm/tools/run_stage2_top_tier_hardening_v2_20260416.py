...
def summarize(args: Any) -> Dict[str, Any]:
    ...
    state_eval = read_json(args.state_ident_eval_v2)
    mechanism_diag = read_json(args.mechanism_fix_v2_diagnosis)
    stage2_qual = read_json(args.stage2_qual_v8)
    ...
    payload = {
        ...
        "sources": {
            ...
            "state_identifiability_eval_v2": str(args.state_ident_eval_v2),
            "mechanism_fix_v2_diagnosis": str(args.mechanism_fix_v2_diagnosis),
            "stage2_qualitative_pack_v8": str(args.stage2_qual_v8),
        ...
        "state_identifiability_protocol_v2_success": bool(state_eval.get("state_identifiability_protocol_v2_success", False)),
        "protocol_v2_discriminative_enough_for_top_tier": bool(state_eval.get("protocol_v2_discriminative_enough_for_top_tier", False)),
...
def diagnose(args: Any) -> Dict[str, Any]:
    ...
    current_stage2_ready_to_freeze = bool(
        ...
        and summary["state_identifiability_protocol_v2_success"]
        and summary["protocol_v2_discriminative_enough_for_top_tier"]
    ...
    if current_stage2_ready_to_freeze:
    ...
    elif not summary["state_identifiability_protocol_v2_success"] or not summary["future_grounding_usefulness_improved_vs_baselines"]:
        next_step_choice = "reconsider_stage2_only_if_identifiability_fails"
        thickness = "cvpr_borderline"
    elif not summary["protocol_v2_discriminative_enough_for_top_tier"]:
        next_step_choice = "run_one_more_protocol_v3_scaleup"
        thickness = "cvpr_borderline"
    elif not mech_all:
        ...
        next_step_choice = "add_one_optional_application_bridge_after_freeze"
        thickness = "oral_ready"
    payload = {
        **summary,
    ...
    write_md(
        args.results_md,
        [
            "# Stage2 Top-tier Hardening V2 20260416",
            "",
            f"- mainline_still_calibration_only: {payload['mainline_still_calibration_only']}",
            f"- state_identifiability_protocol_v2_success: {payload['state_identifiability_protocol_v2_success']}",
            f"- protocol_v2_discriminative_enough_for_top_tier: {payload['protocol_v2_discriminative_enough_for_top_tier']}",
...
def parse_args() -> Any:
    ...
    parser.add_argument("--stage2-qual-v8", default=str(ROOT / "reports/stage2_qualitative_pack_v8_20260416.json"))
    parser.add_argument("--aux-probe-v2", default=str(ROOT / "reports/stage2_aux_external_probe_batch_v2_20260414.json"))
    parser.add_argument("--summary-report", default=str(ROOT / "reports/stage2_top_tier_hardening_v2_summary_20260416.json"))
    parser.add_argument("--diagnosis-report", default=str(ROOT / "reports/stage2_top_tier_hardening_v2_diagnosis_20260416.json"))
    parser.add_argument("--results-md", default=str(ROOT / "docs/STAGE2_TOP_TIER_HARDENING_V2_20260416.md"))
