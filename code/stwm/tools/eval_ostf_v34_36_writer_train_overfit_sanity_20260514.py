...
from stwm.tools.train_eval_ostf_v34_36_predictability_filtered_unit_delta_writer_20260514 import CKPT_DIR, eval_split


REPORT = ROOT / "reports/stwm_ostf_v34_36_writer_train_overfit_sanity_20260514.json"
DOC = ROOT / "docs/STWM_OSTF_V34_36_WRITER_TRAIN_OVERFIT_SANITY_20260514.md"
...
def main() -> int:
    ...
    cache = eval_split("train", model, ckargs, head, readers, args, device)
    rows = []
    ...
    train_overfits_filtered_target = bool((metrics["hard_changed_gain_vs_anchor"] or -1.0) > 0.002 and (delta["shuffle_assignment_delta"] or 0.0) > 0.002)
    payload = {
        ...
        "train_overfits_filtered_target": train_overfits_filtered_target,
        "train_metrics": metrics,
        "train_intervention_delta": delta,
        "best_copy_topk_baseline_train": base,
        "recommended_next_step": "fix_writer_training_or_capacity" if not train_overfits_filtered_target else "fix_writer_generalization",
    }
    dump_json(REPORT, payload)
    write_doc(DOC, "V34.36 writer train-overfit sanity 中文报告", payload, ["中文结论", "selected_train_config", "train_overfits_filtered_target", "train_intervention_delta", "recommended_next_step"])
    print(f"已写出 V34.36 writer train-overfit sanity 报告: {REPORT.relative_to(ROOT)}", flush=True)
    print(f"train_overfits_filtered_target: {train_overfits_filtered_target}", flush=True)
