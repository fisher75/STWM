...
def build_parser() -> ArgumentParser:
    p = ArgumentParser(description="Watch QTSA seed42 matrix and write final report")
    p.add_argument(
        "--submit-tsv",
        default=(
            "/home/chen034/workspace/stwm/reports/"
            "stwm_qtsa_mainline_seed42_submit_v1.tsv"
        ),
    )
    p.add_argument(
        "--queue-status-dir",
    ...
    p.add_argument(
        ...
        default=(
            "/home/chen034/workspace/stwm/reports/"
            "stwm_qtsa_mainline_seed42_report_v1.json"
        ),
    )
    p.add_argument(
        "--out-doc",
        default=(
            ...
            "STWM_QTSA_MAINLINE_SEED42_REPORT_V1.md"
        ),
    )
    return p
...
def _write_markdown(report: dict[str, Any], out_doc: Path) -> None:
    lines: list[str] = []
    lines.append("# STWM QTSA Mainline Seed42 Report V1")
    lines.append("")
    ...
    if isinstance(ranked, list) and ranked:
        lines.append("## Official Ranking (Within This 3-Run QTSA Matrix)")
        lines.append("")
...
def main() -> None:
    ...
    while True:
        ...
        print(f"[qtsa-seed42-watch] {summary}", flush=True)
