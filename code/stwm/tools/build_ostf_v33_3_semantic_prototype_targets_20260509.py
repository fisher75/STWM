...
VOCAB_REPORT = ROOT / "reports/stwm_ostf_v33_3_semantic_prototype_vocab_20260509.json"
OUT_BASE = ROOT / "outputs/cache/stwm_ostf_v33_3_semantic_prototype_targets/pointodyssey/clip_vit_b32_local"
REPORT = ROOT / "reports/stwm_ostf_v33_3_semantic_prototype_targets_20260509.json"
DOC = ROOT / "docs/STWM_OSTF_V33_3_SEMANTIC_PROTOTYPE_TARGETS_20260509.md"
...
def load_selected_k(default: int = 64) -> int:
    if not VOCAB_REPORT.exists():
        return default
    return int(json.loads(VOCAB_REPORT.read_text(encoding="utf-8")).get("selected_K", default))


def top_distribution(emb: np.ndarray, mask: np.ndarray, centers: np.ndarray, topk: int = 8) -> tuple[np.ndarray, np.ndarray]:
    ...
    if flat.shape[0] and centers.shape[0]:
...
def main() -> int:
    ...
    args = p.parse_args()
    k = int(args.K or load_selected_k())
    vocab_path = ROOT / f"outputs/cache/stwm_ostf_v33_3_semantic_prototypes/pointodyssey/clip_vit_b32_local/K{k}/prototype_vocab.npz"
    if not vocab_path.exists():
        payload = {"generated_at_utc": utc_now(), "prototype_targets_built": False, "exact_blocker": f"missing vocab {vocab_path}"}
        dump_json(REPORT, payload)
        write_doc(DOC, "STWM OSTF V33.3 Semantic Prototype Targets", payload, ["prototype_targets_built", "exact_blocker"])
    ...
    for split in ("train", "val", "test"):
        ...
        if args.max_files_per_split is not None:
            files = files[: int(args.max_files_per_split)]
        out_dir = out_root / split
        out_dir.mkdir(parents=True, exist_ok=True)
        count = fut_valid = obs_valid = 0
        for path in files:
            ...
            fut_id, fut_dist = top_distribution(fut, fm, centers)
            obs_id, obs_dist = top_distribution(obs, om, centers)
            sample_uid = str(np.asarray(z["sample_uid"]).item())
            np.savez_compressed(
                out_dir / f"{sample_uid}.npz",
        ...
        totals[split] = {
            "sample_count": count,
            "future_prototype_valid_count": fut_valid,
            "observed_prototype_valid_count": obs_valid,
            "output_dir": str(out_dir.relative_to(ROOT)),
        }
    payload = {
        "generated_at_utc": utc_now(),
        "prototype_targets_built": True,
        "selected_K": k,
    ...
    dump_json(REPORT, payload)
    write_doc(DOC, "STWM OSTF V33.3 Semantic Prototype Targets", payload, ["prototype_targets_built", "selected_K", "prototype_vocab_path", "leakage_safe", "future_prototypes_supervision_only", "future_prototypes_input_allowed", "splits"])
    print(REPORT.relative_to(ROOT))
    return 0
