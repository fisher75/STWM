#!/usr/bin/env python3
"""V35.49 full 325 per-category failure atlas。"""
...
base.V35_45_DECISION = ROOT / "reports/stwm_ostf_v35_49_full_325_raw_video_closure_benchmark_decision_20260516.json"
base.EVAL_REPORT = ROOT / "reports/stwm_ostf_v35_49_full_325_per_category_failure_atlas_eval_20260516.json"
base.DECISION_REPORT = ROOT / "reports/stwm_ostf_v35_49_full_325_per_category_failure_atlas_decision_20260516.json"
base.DOC = ROOT / "docs/STWM_OSTF_V35_49_FULL_325_PER_CATEGORY_FAILURE_ATLAS_DECISION_20260516.md"
base.LOG = ROOT / "outputs/logs/stwm_ostf_v35_49_full_325_per_category_failure_atlas_20260516.log"


def postprocess(code: int) -> int:
    benchmark = json.loads(base.V35_45_DECISION.read_text(encoding="utf-8")) if base.V35_45_DECISION.exists() else {}
    bench_pass = bool(benchmark.get("larger_raw_video_closure_benchmark_passed", False))
    final_code = code
    for path in [base.EVAL_REPORT, base.DECISION_REPORT]:
        if path.exists():
            ...
            if path == base.DECISION_REPORT:
                ...
                if data["atlas_ready"]:
                    data["recommended_next_step"] = "write_v35_49_full_325_final_decision"
                    final_code = 0
            path.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
    base.DOC.write_text(
        "# STWM OSTF V35.49 Full 325 Per-Category Failure Atlas Decision\n\n"
        f"- eval_report: {base.EVAL_REPORT.relative_to(ROOT)}\n"
        f"- decision_report: {base.DECISION_REPORT.relative_to(ROOT)}\n"
        f"- m128_h32_full_325_benchmark_passed: {bench_pass}\n"
        "- train_new_model: false\n"
        "- full_cvpr_scale_claim_allowed: false\n\n"
        "## 中文总结\n"
        "V35.49 已对 full 325 raw-video closure slice 做 per-category failure atlas；用于判断 full M128/H32 claim boundary。\n",
        encoding="utf-8",
    )
    return final_code
