...
CKPT_DIR = ROOT / "outputs/checkpoints/stwm_ostf_v34_5_delta_residual_probe_h32_m128"
SUMMARY = ROOT / "reports/stwm_ostf_v34_5_delta_residual_probe_train_summary_20260511.json"
DOC = ROOT / "docs/STWM_OSTF_V34_5_DELTA_RESIDUAL_PROBE_TRAIN_SUMMARY_20260511.md"
...
class StrictResidualUtilityDataset(V341Dataset):
    def __init__(self, split: str, args: argparse.Namespace, *, max_items: int | None = None) -> None:
        ...
        for entry in self.base.entries:
            ...
            if (self.strict_root / split / f"{uid}.npz").exists():
                keep.append(entry)
        self.base.entries = keep[:max_items] if max_items is not None else keep
        if not self.base.entries:
            raise RuntimeError(f"No V34.5 strict residual utility targets for split={split}")

    def __getitem__(self, idx: int) -> dict[str, Any]:
        item = super().__getitem__(idx)
        uid = str(item["uid"])
        z = np.load(self.strict_root / self.split / f"{uid}.npz", allow_pickle=True)
        for key in [
            "pointwise_semantic_cosine",
            "pointwise_semantic_margin",
            ...
            "strict_residual_identity_utility_mask",
            "strict_stable_suppress_mask",
            "strict_residual_gate_target",
            "strict_residual_gate_available_mask",
        ]:
...
def collate_v345(batch: list[dict[str, Any]]) -> dict[str, Any]:
...
def make_loader(split: str, args: argparse.Namespace, *, shuffle: bool) -> DataLoader:
    ...
    return DataLoader(ds, batch_size=args.batch_size, shuffle=shuffle, num_workers=args.num_workers, pin_memory=torch.cuda.is_available(), collate_fn=collate_v345)
...
def delta_target(base: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    base_n = F.normalize(base.detach(), dim=-1)
    target_n = F.normalize(torch.nan_to_num(target.float()), dim=-1)
    alpha = (target_n * base_n).sum(dim=-1, keepdim=True)
    delta = target_n - alpha * base_n
    return F.normalize(delta, dim=-1)


def delta_oracle_outputs(out: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
    sem_gate = batch["strict_residual_semantic_utility_mask"].float()
    id_gate = batch["strict_residual_identity_utility_mask"].float()
...
def loss_fn(out: dict[str, torch.Tensor], batch: dict[str, torch.Tensor], args: argparse.Namespace) -> tuple[torch.Tensor, dict[str, float]]:
    final_sem, final_id = delta_oracle_outputs(out, batch)
    ...
    delta = delta_target(base, target)
    final_loss = cosine_loss(final_sem, target, sem_pos, sem_weight) if bool(sem_pos.any()) else final_sem.sum() * 0.0
    direction = cosine_loss(out["unit_semantic_residual"], delta, sem_pos, sem_weight) if bool(sem_pos.any()) else final_sem.sum() * 0.0
    residual_n = F.normalize(out["unit_semantic_residual"], dim=-1)
    base_n = F.normalize(base, dim=-1)
    orth = ((residual_n * base_n).sum(dim=-1).abs() * sem_pos.float()).sum() / sem_pos.float().sum().clamp_min(1.0)
    base_cos = (F.normalize(base, dim=-1) * F.normalize(torch.nan_to_num(target.float()), dim=-1)).sum(dim=-1).detach()
    final_cos = (F.normalize(final_sem, dim=-1) * F.normalize(torch.nan_to_num(target.float()), dim=-1)).sum(dim=-1)
    ...
    id_pos = batch["strict_residual_identity_utility_mask"].bool() & batch["fut_instance_available_mask"].bool()
    ...
    total = (
        ...
        + args.delta_direction_weight * direction
        + args.orthogonality_weight * orth
    ...
    stats = {
        ...
        "delta_residual_direction_loss": float(direction.detach().cpu()),
        "residual_orthogonality": float(orth.detach().cpu()),
        "semantic_hard_gain_objective": float(gain_loss.detach().cpu()),
        "stable_preservation_loss": float(stable_loss.detach().cpu()),
        "residual_identity_diagnostic_loss": float(id_loss.detach().cpu()),
...
def train_one(args: argparse.Namespace) -> dict[str, Any]:
    ...
    ckpt = CKPT_DIR / "v34_5_delta_residual_probe_m128_h32_seed42_best.pt"
    ...
    payload = {
        ...
        "delta_residual_objective_active": True,
    ...
    write_doc(DOC, "STWM OSTF V34.5 Delta Residual Probe Train Summary", payload, ["delta_residual_probe_ran", "fresh_training_completed", "checkpoint_path", "train_sample_count", "v30_backbone_frozen", "pointwise_base_frozen", "delta_residual_objective_active", "future_leakage_detected", "train_loss_decreased"])
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--final-target-weight", type=float, default=1.1)
    p.add_argument("--delta-direction-weight", type=float, default=1.0)
    p.add_argument("--orthogonality-weight", type=float, default=0.2)
