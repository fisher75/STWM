...
from stwm.tools.train_ostf_v34_20_hard_changed_aligned_topk_residual_probe_20260513 import CKPT_DIR, SUMMARY as TRAIN_SUMMARY, compose, hard_changed_aligned_mask


SUMMARY = ROOT / "reports/stwm_ostf_v34_20_hard_changed_aligned_topk_residual_probe_eval_summary_20260513.json"
DECISION = ROOT / "reports/stwm_ostf_v34_20_hard_changed_aligned_topk_residual_probe_decision_20260513.json"
DOC = ROOT / "docs/STWM_OSTF_V34_20_HARD_CHANGED_ALIGNED_TOPK_RESIDUAL_PROBE_DECISION_20260513.md"
...
class UnitStats:
    ...
    def update(self, out: dict[str, torch.Tensor], inst: torch.Tensor, obs_sem: torch.Tensor, obs_mask: torch.Tensor, pos: torch.Tensor) -> None:
        ...
        sem_cpu = F.normalize(sem_pool, dim=-1).detach().cpu().numpy()
...
def finalize(acc: Acc, prefix: str) -> dict[str, Any]:
    keys = ["aligned", "causal", "strict", "hard", "changed", "hard_changed", "stable", "valid"]
...
def load_model(args: argparse.Namespace, device: torch.device) -> tuple[TopKEvidenceResidualMemoryV3418 | None, argparse.Namespace | None, dict[str, Any]]:
    ...
    model = TopKEvidenceResidualMemoryV3418(ckargs.v30_checkpoint, teacher_embedding_dim=ckargs.teacher_embedding_dim, units=ckargs.trace_units, horizon=ckargs.horizon, selector_hidden_dim=ckargs.selector_hidden_dim, topk=ckargs.topk).to(device)
...
def split_eval(split: str, model: TopKEvidenceResidualMemoryV3418, ckargs: argparse.Namespace, device: torch.device) -> dict[str, Any]:
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            unit.update(outs["normal"], bd["point_to_instance_id"], bd["obs_semantic_measurements"], bd["obs_semantic_measurement_mask"], masks["aligned"])
    per = {mode: finalize(accs[mode], mode) for mode in modes}
    normal = per["normal"]
    deltas = {
        "zero_semantic_measurements_metric_delta": None if normal["aligned_gain"] is None or per["zero_semantic_measurements"]["aligned_gain"] is None else float(normal["aligned_gain"] - per["zero_semantic_measurements"]["aligned_gain"]),
        "shuffle_semantic_measurements_metric_delta": None if normal["aligned_gain"] is None or per["shuffle_semantic_measurements"]["aligned_gain"] is None else float(normal["aligned_gain"] - per["shuffle_semantic_measurements"]["aligned_gain"]),
        "shuffle_assignment_metric_delta": None if normal["aligned_gain"] is None or per["shuffle_assignment"]["aligned_gain"] is None else float(normal["aligned_gain"] - per["shuffle_assignment"]["aligned_gain"]),
        "zero_unit_memory_metric_delta": None if normal["aligned_gain"] is None or per["zero_unit_memory"]["aligned_gain"] is None else float(normal["aligned_gain"] - per["zero_unit_memory"]["aligned_gain"]),
        "selector_ablation_delta": None if normal["aligned_gain"] is None or per["selector_ablation"]["aligned_gain"] is None else float(normal["aligned_gain"] - per["selector_ablation"]["aligned_gain"]),
    }
...
def main() -> int:
    ...
    if model is None or ckargs is None:
        decision = {
            ...
            "中文结论": "V34.20 hard/changed-aligned top-k residual 训练未运行，评估跳过。",
        ...
        write_doc(DOC, "V34.20 hard/changed aligned top-k residual probe 决策中文报告", decision, ["中文结论", "oracle_residual_probe_ran", "oracle_residual_probe_passed", "skip_reason", "recommended_next_step"])
        print(f"已写出 V34.20 评估跳过报告: {DECISION.relative_to(ROOT)}")
        return 0
    ...
    passed = bool(
        bool(model.v30_backbone_frozen)
        and sem_lb
        and assign_lb
        and unit_lb
        and semantic_signal
        ...
        and (nval["aligned_gain"] or 0.0) > 0
        and (ntest["aligned_gain"] or 0.0) > 0
    )
    decision = {
        "generated_at_utc": utc_now(),
        ...
        "hard_changed_aligned_gain": {"val": nval["aligned_gain"], "test": ntest["aligned_gain"]},
        "causal_assignment_subset_gain": {"val": nval["causal_gain"], "test": ntest["causal_gain"]},
        "strict_residual_subset_gain": {"val": nval["strict_gain"], "test": ntest["strict_gain"]},
        ...
        "recommended_next_step": "build_observed_only_hard_changed_gate_target" if passed else "fix_hard_changed_aligned_topk_residual_content",
    }
    payload = {
        "generated_at_utc": utc_now(),
        "train_summary": train,
        "v34_18_reference_decision": json.loads(V3418_DECISION.read_text(encoding="utf-8")) if V3418_DECISION.exists() else {},
        "v34_19_reference": json.loads(V3419.read_text(encoding="utf-8")) if V3419.exists() else {},
        "per_split": per,
        "decision": decision,
    }
    ...
    print(f"已写出 V34.20 hard/changed aligned top-k residual 评估摘要: {SUMMARY.relative_to(ROOT)}")
    return 0
