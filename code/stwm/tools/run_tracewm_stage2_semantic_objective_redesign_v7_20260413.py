...
def summarize(args: Any) -> Dict[str, Any]:
    ...
    for meta in launch.get("runs", []) if isinstance(launch.get("runs", []), list) else []:
        ...
        declared = bool(detail.get("persistence_objective_declared", meta.get("persistence_objective_declared", False)))
        ...
        persistence_effective = bool((not declared) or (guaranteed_pair_count >= 1.0 and valuable_pair_ratio > 0.0))
        rows.append(
            {
                "run_name": str(meta.get("run_name", "")),
                ...
                "selected_gpu_id": int(detail.get("selected_gpu_id", meta.get("selected_gpu_id", -1))),
                "lease_id": str(detail.get("lease_id", meta.get("lease_id", ""))),
    ...
    payload = {
        "generated_at_utc": now_iso(),
        "redesign_v7_status": f"{running}_running_{completed}_completed_{failed}_failed",
        "runs": rows,
        "next_step_choice_internal": "summarize_redesign_v7_after_completion" if completed == len(rows) and rows and failed == 0 else ("fix_failed_v7_runs" if failed else "continue_redesign_v7"),
    }
    ...
    lines = [
        ...
        f"- redesign_v7_status: {payload['redesign_v7_status']}",
        "",
    ...
    for row in rows:
        ...
        lines.append(
            f"| {row['run_name']} | {row.get('objective_family', '')} | {row['objective_combo']} | {row['v6_gating_family']} | {row['selected_gpu_id']} | {row['batch_size']} | {row['train_steps']} | {row['status']} | {bool(row.get('persistence_objective_declared', False))} | {bool(row.get('persistence_declared_but_inactive', False))} | {_f(best.get('free_rollout_endpoint_l2'), 1e9):.8f} | {float(row.get('actual_gate_positive_ratio_mean', 0.0)):.4f} | {float(row.get('guaranteed_pair_count_mean', 0.0)):.4f} | {float(row.get('valuable_pair_ratio_mean', 0.0)):.4f} | {float(row.get('strict_pair_ratio_mean', 0.0)):.4f} | {float(row.get('fallback_trigger_rate_mean', 0.0)):.4f} | {float(row.get('fallback_pair_ratio_mean', 0.0)):.4f} |"
        )
...
def diagnose_v7(args: Any) -> Dict[str, Any]:
    ...
    completed = [r for r in rows if str(r.get("status", "")).lower() == "completed"]

    refs = _baseline_refs()
    v5 = _read_json(args.v5_reference_diagnosis_report) if Path(args.v5_reference_diagnosis_report).exists() else {}
    v6 = _read_json(args.v6_reference_diagnosis_report) if Path(args.v6_reference_diagnosis_report).exists() else {}
    ...
    v5_best_full_ep = _f(v5.get("success_criteria", {}).get("best_v5_full_validation_endpoint_l2"), 1e9)
    v5_best_hard_comp = _f(v5.get("success_criteria", {}).get("best_v5_semantic_hard_composite_score"), 1e9)
    v6_best_full_ep = _f(v6.get("success_criteria", {}).get("best_v6_full_validation_endpoint_l2"), 1e9)
    v6_best_hard_comp = _f(v6.get("success_criteria", {}).get("best_v6_semantic_hard_composite_score"), 1e9)

    payload: Dict[str, Any] = {
        "generated_at_utc": now_iso(),
        "diagnosis_type": "stage2_semantic_objective_redesign_v7",
        "teacher_as_mainline_semantic_source": False,
        "chosen_bootstrap_backend": "local_clip_vit_b32_mask_crop_visual_teacher",
        "v7_runs_terminal": bool(rows and len(completed) == len(rows)),
        ...
        "baseline_refs": {
            ...
            "v5_best_objective_combo_endpoint_l2": float(v5_best_full_ep),
            "v5_best_semantic_hard_composite_score": float(v5_best_hard_comp),
            "v6_best_objective_combo_endpoint_l2": float(v6_best_full_ep),
            "v6_best_semantic_hard_composite_score": float(v6_best_hard_comp),
    ...
    if not payload["v7_runs_terminal"]:
        payload["success_criteria"] = {
            "semantic_hard_composite_improved_vs_v5": False,
            "improved_vs_v5_best_objective_combo": False,
            "alignment_only_is_already_sufficient": False,
            "persistence_branch_actually_contributed": False,
            "full_validation_non_catastrophic": False,
            "persistence_declared_but_inactive_any": False,
            "persistence_declared_but_inactive_all": False,
            "best_v7_objective_combo": "none",
            "best_v7_full_validation_endpoint_l2": 1e9,
            "best_v7_semantic_hard_composite_score": 1e9,
            "best_alignment_only_run_name": "none",
            "best_persistence_run_name": "none",
    ...
    if bool(args.diagnose_use_final_metrics_only):
        full_rows: List[Dict[str, Any]] = []
        hard_rows: List[Dict[str, Any]] = []
        for row in completed:
            run_name = str(row.get("run_name", ""))
            final = _read_json(str(row.get("final_json", "")))
            best_block = final.get("best_checkpoint_metric", {}) if isinstance(final.get("best_checkpoint_metric", {}), dict) else {}
            best_metrics = best_block.get("metrics", {}) if isinstance(best_block.get("metrics", {}), dict) else {}
            sidecar = final.get("semantic_hard_sidecar_metric", {}) if isinstance(final.get("semantic_hard_sidecar_metric", {}), dict) else {}
            sidecar_score = _f(sidecar.get("semantic_hard_sidecar_score"), 1e9)
            if sidecar_score >= 1e8:
                sidecar_score = float(0.7 * _f(best_metrics.get("free_rollout_endpoint_l2"), 1e9) + 0.3 * _f(best_metrics.get("free_rollout_coord_mean_l2"), 1e9))

            full_rows.append(
                {
                    "run_name": run_name,
                    "objective_combo": str(row.get("objective_combo", "")),
                    "objective_family": str(row.get("objective_family", "")),
                    "seed": int(row.get("seed", -1)),
                    "metrics": {k: _f(best_metrics.get(k), 1e9) for k in METRIC_KEYS},
                }
            )
            hard_rows.append(
                {
                    ...
                    "objective_family": str(row.get("objective_family", "")),
                    "seed": int(row.get("seed", -1)),
                    "semantic_hard_composite_score": float(sidecar_score),
                    "persistence_objective_declared": bool(row.get("persistence_objective_declared", False)),
                    "persistence_declared_but_inactive": bool(row.get("persistence_declared_but_inactive", False)),
                    "actual_gate_positive_ratio": float(row.get("actual_gate_positive_ratio_mean", 0.0)),
                    "valuable_pair_ratio": float(row.get("valuable_pair_ratio_mean", 0.0)),
                    "fallback_trigger_rate": float(row.get("fallback_trigger_rate_mean", 0.0)),
                    "guaranteed_pair_count": float(row.get("guaranteed_pair_count_mean", 0.0)),
                    "strict_pair_ratio": float(row.get("strict_pair_ratio_mean", 0.0)),
                    "fallback_pair_ratio": float(row.get("fallback_pair_ratio_mean", 0.0)),
                    "same_checkpoint_selected": bool(row.get("same_checkpoint_selected", False)),
                    "sidecar_truly_diverged": bool(row.get("sidecar_truly_diverged", False)),
                }
            )
        ...
        payload["evaluation_cache"] = {
            ...
            "cache_misses": 0,
            "cached_run_count": 0,
            "mode": "derived_from_final_metrics_only",
        }
        payload["full_validation_panel"] = {
            "runs": full_rows,
            "aggregate": _aggregate_rows(full_rows),
            "dataset_binding": ["VSPW", "VIPSeg"],
            "eval_scope": "best_checkpoint_metric_from_run_final_json",
        }
        payload["semantic_hard_subset_panel"] = {
            "status": "omitted_in_v7_fast_diagnosis",
            "reason": "use per-run semantic_hard_sidecar_score for disambiguation",
        }
        payload["burst_persistence_hard_panel"] = {
            "status": "skipped_in_v7_fast_diagnosis",
            "reason": "not required for alignment-vs-persistence final disambiguation",
        }
        payload["semantic_hard_composite_panel"] = {"runs": hard_rows}

        run_to_full_ep = {str(x.get("run_name", "")): _f(x.get("metrics", {}).get("free_rollout_endpoint_l2"), 1e9) for x in full_rows}
        run_to_hard_comp = {str(x.get("run_name", "")): _f(x.get("semantic_hard_composite_score"), 1e9) for x in hard_rows}
        run_to_completed_row = {str(x.get("run_name", "")): x for x in completed}
        ...
        best_combo = str(run_to_completed_row.get(best_hard_run, {}).get("objective_combo", "none"))

        align_runs = [r for r in completed if str(r.get("objective_family", "")) == "calibration_only_family"]
        persist_runs = [r for r in completed if str(r.get("objective_family", "")) == "calibration_plus_active_persistence_family"]
        persist_effective_runs = [r for r in persist_runs if not bool(r.get("persistence_declared_but_inactive", False))]

        def _best_run_name(rows_local: List[Dict[str, Any]]) -> str:
            if not rows_local:
                return "none"
            return min(rows_local, key=lambda x: run_to_hard_comp.get(str(x.get("run_name", "")), 1e9)).get("run_name", "none")

        align_best_run = str(_best_run_name(align_runs))
        persist_best_run = str(_best_run_name(persist_runs))
        persist_effective_best_run = str(_best_run_name(persist_effective_runs))

        align_best_hard = float(run_to_hard_comp.get(align_best_run, 1e9))
        align_best_full = float(run_to_full_ep.get(align_best_run, 1e9))
        persist_best_hard = float(run_to_hard_comp.get(persist_best_run, 1e9))
        persist_best_full = float(run_to_full_ep.get(persist_best_run, 1e9))
        persist_effective_best_hard = float(run_to_hard_comp.get(persist_effective_best_run, 1e9))
        persist_effective_best_full = float(run_to_full_ep.get(persist_effective_best_run, 1e9))

        semantic_hard_improved_vs_v5 = bool(best_hard_composite < float(v5_best_hard_comp) * 0.98)
        improved_vs_v5_combo = bool(best_full_endpoint < float(v5_best_full_ep))
        full_non_catastrophic = bool(best_full_endpoint <= float(crop_ep) * 1.5)
        persistence_contributed = bool(
            persist_effective_best_run != "none"
            and align_best_run != "none"
            and persist_effective_best_hard < align_best_hard * 0.98
            and persist_effective_best_full <= align_best_full * 1.02
        )
        alignment_only_sufficient = bool(
            align_best_run != "none"
            and align_best_hard < float(v5_best_hard_comp) * 0.98
            and align_best_full <= float(crop_ep) * 1.5
            and (not persistence_contributed)
        )

        persistence_declared_but_inactive_any = bool(any(bool(r.get("persistence_declared_but_inactive", False)) for r in persist_runs))
        persistence_declared_but_inactive_all = bool(bool(persist_runs) and all(bool(r.get("persistence_declared_but_inactive", False)) for r in persist_runs))

        if alignment_only_sufficient and not persistence_contributed:
            next_step = "alignment_only_is_true_mainline"
        elif persistence_contributed:
            next_step = "persistence_is_load_bearing"
        elif persistence_declared_but_inactive_all:
            next_step = "persistence_declared_but_not_active"
        else:
            next_step = "still_cannot_disambiguate"

        payload["family_disambiguation_panel"] = {
            "alignment_only_family": {
                "best_run_name": str(align_best_run),
                "best_full_validation_endpoint_l2": float(align_best_full),
                "best_semantic_hard_composite_score": float(align_best_hard),
            },
            "calibration_plus_active_persistence_family": {
                "best_run_name": str(persist_best_run),
                "best_full_validation_endpoint_l2": float(persist_best_full),
                "best_semantic_hard_composite_score": float(persist_best_hard),
                "best_effective_run_name": str(persist_effective_best_run),
                "best_effective_full_validation_endpoint_l2": float(persist_effective_best_full),
                "best_effective_semantic_hard_composite_score": float(persist_effective_best_hard),
            },
        }
        payload["success_criteria"] = {
            "semantic_hard_composite_improved_vs_v5": bool(semantic_hard_improved_vs_v5),
            "improved_vs_v5_best_objective_combo": bool(improved_vs_v5_combo),
            "alignment_only_is_already_sufficient": bool(alignment_only_sufficient),
            "persistence_branch_actually_contributed": bool(persistence_contributed),
            "full_validation_non_catastrophic": bool(full_non_catastrophic),
            ...
            "improved_vs_current_cropenc_baseline": bool(best_full_endpoint < float(crop_ep)),
            "improved_vs_v6_best_objective_combo": bool(best_hard_composite < float(v6_best_hard_comp)),
            "best_v7_objective_combo": str(best_combo),
            "best_v7_full_validation_endpoint_l2": float(best_full_endpoint),
            "best_v7_semantic_hard_composite_score": float(best_hard_composite),
            "best_alignment_only_run_name": str(align_best_run),
            "best_persistence_run_name": str(persist_best_run),
            "best_effective_persistence_run_name": str(persist_effective_best_run),
    ...
    try:
        print(f"[semobjv7] diagnosis device={device} eval_gpu={eval_gpu}", flush=True)
        ...
        print("[semobjv7] optional BURST persistence-hard eval", flush=True)
        burst_eval_ds = None
        ...
        for row in completed:
            ...
            cache_misses += 1
            print(f"[semobjv7] full validation eval run={run_name}", flush=True)
            ...
            try:
                ...
                full_metrics_payload = {k: _f(full_metrics.get(k), 1e9) for k in METRIC_KEYS}
                hard_metrics_payload: Dict[str, Dict[str, Any]] = {}
                for subset_name in hard_subset_names:
                    print(f"[semobjv7] semantic hard eval subset={subset_name} run={run_name}", flush=True)
            ...
            _write_json(
                ...
                {
                    "generated_at_utc": now_iso(),
                    "cache_type": "stage2_semantic_objective_redesign_v7_diagnosis_cache",
            ...
            "runs": [
                {
                    ...
                    "objective_family": str(run_aux.get(run_name, {}).get("objective_family", "")),
                    ...
                    "persistence_objective_declared": bool(run_aux.get(run_name, {}).get("persistence_objective_declared", False)),
                    "persistence_declared_but_inactive": bool(run_aux.get(run_name, {}).get("persistence_declared_but_inactive", False)),
    ...
    run_to_full_row = {str(row["run_name"]): row for row in payload["full_validation_panel"]["runs"]}
    run_to_completed_row = {str(row.get("run_name", "")): row for row in completed}
    ...
    persistence_declared_but_inactive_any = bool(
        any(bool(r.get("persistence_declared_but_inactive", False)) for r in persist_runs)
    )
    persistence_declared_but_inactive_all = bool(
        bool(persist_runs)
        and all(bool(r.get("persistence_declared_but_inactive", False)) for r in persist_runs)
    )
