...
from stwm.tools.train_ostf_context_residual_v20_20260502 import (
    _combo_for_model,
    _evaluate_model,
    _build_model,
)
...
TRAIN_RE = re.compile(r"\[V20\]\[train\]\s+(.*)")
VAL_RE = re.compile(r"\[V20\]\[val\]\s+(.*)")
...
def main() -> int:
    ...
    report = {
        ...
        "metric_note": "V20 finalized from saved best checkpoint after live training; semantic metrics remain secondary because OSTF target semantics are mostly static.",
    }
