...
@dataclass
class OSTFCalibratedMultimodalConfig:
    obs_len: int = 8
    horizon: int = 8
    point_dim: int = 224
    hidden_dim: int = 384
    ...
    point_meta_dim: int = 6
    use_context: bool = True
    use_dense_points: bool = True
    use_semantic_memory: bool = True
    use_cv_mode: bool = True
    use_affine_prior: bool = True
    use_cv_prior: bool = True


class PointTraceEncoderV22(nn.Module):
    def __init__(self, obs_len: int, point_dim: int, point_meta_dim: int) -> None:
        super().__init__()
        in_dim = obs_len * 7 + point_meta_dim
    ...
    def forward(
        ...
        flat = feat.reshape(obs_points.shape[0], obs_points.shape[1], -1)
        return self.net(torch.cat([flat, point_meta], dim=-1))


class ContextTokenEncoderV22(nn.Module):
...
class StepContextRefinerV22(nn.Module):
    ...
    def forward(self, point_tokens: torch.Tensor, future_token: torch.Tensor, context_tokens: torch.Tensor, semantic_token: torch.Tensor) -> torch.Tensor:
        future = future_token[:, None, :]
        semantic = semantic_token[:, None, :]
        seq = torch.cat([future, semantic, context_tokens, point_tokens], dim=1)
        out = self.encoder(seq)
        return out[:, 2 + context_tokens.shape[1] :, :]


class OSTFCalibratedMultimodalWorldModel(nn.Module):
    def __init__(self, cfg: OSTFCalibratedMultimodalConfig) -> None:
        super().__init__()
        self.cfg = cfg
        self.point_encoder = PointTraceEncoderV22(cfg.obs_len, cfg.point_dim, cfg.point_meta_dim)
        self.point_to_hidden = nn.Linear(cfg.point_dim, cfg.hidden_dim)
        self.context_encoder = ContextTokenEncoderV22(cfg.hidden_dim, cfg.semantic_dim)
        ...
        self.refiner = StepContextRefinerV22(cfg.hidden_dim, cfg.num_heads, cfg.refinement_layers)
        self.mode_condition = nn.Sequential(
    ...
    def _init_parameters(self) -> None:
        ...
        nn.init.zeros_(self.prior_mix_head.bias)
        with torch.no_grad():
    ...
    def forward(
        ...
        *,
        ...
        neighbor_feat: torch.Tensor,
        global_feat: torch.Tensor,
        semantic_aux_feat: torch.Tensor | None = None,
    ) -> dict[str, torch.Tensor]:
        if self.cfg.use_dense_points:
            point_tokens = self.point_encoder(obs_points, obs_vis, rel_xy, point_meta)
        else:
        ...
        if semantic_aux_feat is not None:
            semantic_input = semantic_input + semantic_aux_feat
        semantic_token = self.semantic_proj(semantic_input)
        ...
        elif self.cfg.use_affine_prior:
            learned_prior = point_affine
            prior_mix = torch.cat(
                [
        ...
        refined_steps = [self.refiner(point_hidden, future_tokens[:, t], context_tokens, semantic_token) for t in range(self.cfg.horizon)]
        refined_tokens = torch.stack(refined_steps, dim=2)
        rel_expand = rel_xy[:, :, None, None, :].expand(-1, -1, self.cfg.horizon, self.cfg.num_hypotheses, -1)
        semantic_mode = semantic_token[:, None, None, None, :].expand(-1, refined_tokens.shape[1], self.cfg.horizon, self.cfg.num_hypotheses, -1)
        mode_tokens = refined_tokens[:, :, :, None, :] + self.mode_embed[None, None, None, :, :]
        mode_tokens = mode_tokens + self.mode_condition(torch.cat([mode_tokens, semantic_mode], dim=-1))
        delta = 0.12 * torch.tanh(self.delta_head(torch.cat([mode_tokens, rel_expand], dim=-1)))
        residual_scale = 0.005 + 0.095 * torch.sigmoid(self.global_residual_scale)
        point_hyp = learned_prior[:, :, :, None, :] + residual_scale * delta

        if self.cfg.use_cv_mode and self.cfg.num_hypotheses >= 1:
            point_hyp = point_hyp.clone()
            point_hyp[:, :, :, 0, :] = point_cv
        point_hyp = torch.nan_to_num(point_hyp, nan=0.0, posinf=1.25, neginf=-0.25).clamp(-0.25, 1.25)

        hyp_context = torch.cat([future_tokens.mean(dim=1), context_summary, point_summary, semantic_token], dim=-1)
        hyp_logits_raw = torch.nan_to_num(self.hypothesis_score_head(hyp_context), nan=0.0, posinf=8.0, neginf=-8.0)
        if self.cfg.num_hypotheses == 1:
            hyp_logits_raw = hyp_logits_raw.new_zeros(hyp_logits_raw.shape[0], 1)
        temperature = 0.5 + torch.nn.functional.softplus(self.logit_temperature)
        hyp_logits = hyp_logits_raw / temperature
        hyp_weight = torch.softmax(hyp_logits, dim=-1)
        point_pred = (point_hyp * hyp_weight[:, None, None, :, None]).sum(dim=3)
        top1_idx = hyp_weight.argmax(dim=-1)
        gather_idx = top1_idx[:, None, None, None, None].expand(-1, point_hyp.shape[1], point_hyp.shape[2], 1, point_hyp.shape[4])
        top1_pred = point_hyp.gather(3, gather_idx).squeeze(3)

        point_pred = torch.nan_to_num(point_pred, nan=0.0, posinf=1.25, neginf=-0.25).clamp(-0.25, 1.25)
        top1_pred = torch.nan_to_num(top1_pred, nan=0.0, posinf=1.25, neginf=-0.25).clamp(-0.25, 1.25)
        anchor_pred = top1_pred.mean(dim=1) + 0.02 * torch.tanh(self.anchor_res_head(future_tokens))
        anchor_pred = torch.nan_to_num(anchor_pred, nan=0.0, posinf=1.25, neginf=-0.25).clamp(-0.25, 1.25)
        visibility_logits = self.visibility_head(
            torch.cat([refined_tokens, obs_vis[:, :, -1:, None].float().expand(-1, -1, self.cfg.horizon, -1)], dim=-1)
        ).squeeze(-1)
        visibility_logits = torch.nan_to_num(visibility_logits, nan=0.0, posinf=8.0, neginf=-8.0)
        semantic_logits = self.semantic_head(torch.cat([future_tokens, context_summary[:, None, :].expand(-1, self.cfg.horizon, -1)], dim=-1))
        semantic_logits = torch.nan_to_num(semantic_logits, nan=0.0, posinf=8.0, neginf=-8.0)
        logvar_modes = self.logvar_head(mode_tokens).squeeze(-1).mean(dim=1)
        logvar_modes = torch.clamp(torch.nan_to_num(logvar_modes, nan=0.0, posinf=2.0, neginf=-6.0), min=-6.0, max=2.0)
        return {
            "point_cv": point_cv,
            "point_affine": point_affine,
            "point_learned_prior": learned_prior,
            "prior_mix": prior_mix,
            "point_hypotheses": point_hyp,
            "point_pred": point_pred,
            "top1_point_pred": top1_pred,
            "hypothesis_logits": hyp_logits,
            "hypothesis_logits_raw": hyp_logits_raw,
            "hypothesis_temperature": torch.as_tensor(temperature, device=obs_points.device, dtype=obs_points.dtype),
            "mode_logvar": logvar_modes,
