...
class OSTFMultimodalWorldModel(nn.Module):
    def __init__(self, cfg: OSTFMultimodalConfig) -> None:
        super().__init__()
        self.cfg = cfg
    ...
    def forward(
        ...
        *,
        ...
        delta = 0.10 * torch.tanh(self.delta_head(torch.cat([mode_tokens, rel_expand], dim=-1)))
        ...
        hyp_context = torch.cat([future_tokens.mean(dim=1), context_summary, point_summary], dim=-1)
        hyp_logits = torch.nan_to_num(self.hypothesis_score_head(hyp_context), nan=0.0, posinf=8.0, neginf=-8.0)
