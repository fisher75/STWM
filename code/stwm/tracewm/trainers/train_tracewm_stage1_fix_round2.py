...
class Round2TraceModel(nn.Module):
...
def compute_train_losses(
    ...
    if loss_mode == "standard_global_mse":
    ...
    if loss_mode != "dataset_normalized":
...
def build_parser() -> ArgumentParser:
    p = ArgumentParser(description="TraceWM Stage1 model-fix round2 trainer")
    p.add_argument(
        "--fix-mode",
        required=True,
        choices=[
            "joint_balanced_lossnorm",
            "point_warmup_then_joint_balanced_lossnorm",
            "kubric_warmup_then_joint_balanced_lossnorm",
        ],
    )
...
def _resolve_warmup_source(fix_mode: str) -> str:
    if fix_mode == "point_warmup_then_joint_balanced_lossnorm":
        return "pointodyssey"
    if fix_mode == "kubric_warmup_then_joint_balanced_lossnorm":
        return "kubric"
    return ""


def _effective_warmup_epochs(fix_mode: str, warmup_epochs: int, total_epochs: int) -> int:
    if fix_mode == "joint_balanced_lossnorm":
        return 0
    return max(0, min(int(warmup_epochs), int(total_epochs)))
...
def main() -> int:
    ...
    joint_train_dataset = Stage1UnifiedDataset(
    ...
    joint_val_dataset = Stage1UnifiedDataset(
        dataset_names=["pointodyssey", "kubric"],
    ...
    point_train_dataset = Stage1UnifiedDataset(
        dataset_names=["pointodyssey"],
    ...
    kubric_train_dataset = Stage1UnifiedDataset(
        dataset_names=["kubric"],
    ...
    joint_train_loader, sampler_info = build_balanced_joint_loader(joint_train_dataset, batch_size=args.batch_size)
    ...
    kubric_train_loader = DataLoader(
        kubric_train_dataset,
        batch_size=args.batch_size,
        shuffle=True,
        num_workers=0,
        collate_fn=stage1_collate_fn,
    ...
    model = Round2TraceModel(state_dim=5, hidden_dim=args.hidden_dim).to(device)
    ...
    if (resume_path is not None and resume_path.exists()):
        ...
        phase_schedule = list(payload.get("phase_schedule", []))
        best_val_total_loss = float(payload.get("best_val_total_loss", float("inf")))
        print(f"[fix2] resumed from --resume: {resume_path}")
        ...
        phase_schedule = list(payload.get("phase_schedule", []))
        best_val_total_loss = float(payload.get("best_val_total_loss", float("inf")))
        print(f"[fix2] resumed from latest: {latest_ckpt}")

    joint_train_iter = itertools.cycle(joint_train_loader)
    point_train_iter = itertools.cycle(point_train_loader)
    kubric_train_iter = itertools.cycle(kubric_train_loader)

    warmup_source = _resolve_warmup_source(args.fix_mode)
    warmup_epochs = _effective_warmup_epochs(args.fix_mode, args.warmup_epochs, args.epochs)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch < warmup_epochs:
            phase = "warmup"
            if warmup_source == "pointodyssey":
                active_iter = point_train_iter
            elif warmup_source == "kubric":
                active_iter = kubric_train_iter
            else:
                active_iter = joint_train_iter
            active_loss_mode = "standard_global_mse"
            active_dataset = warmup_source or "joint"
        else:
            phase = "joint"
            active_iter = joint_train_iter
            active_loss_mode = "dataset_normalized"
            active_dataset = "pointodyssey+kubric"
        ...
        phase_schedule.append(
            {
                "epoch": epoch,
                "phase": phase,
                "phase_dataset": active_dataset,
                "phase_loss_mode": active_loss_mode,
            }
        )
        ...
        print(
            f"[fix2][{args.run_name}] epoch={epoch} phase={phase} step={global_step} "
    ...
    summary = {
        ...
        "fix_flags": {
            ...
            "warmup_source": warmup_source if warmup_source else "none",
        },
        ...
        "sampler_info": {
            ...
            "warmup": {
                ...
                "source": warmup_source if warmup_source else "none",
            },
        },
        "loss_family": {
            ...
            "free_loss_weight": args.free_loss_weight,
            "warmup_loss_mode": "standard_global_mse",
            "joint_loss_mode": "dataset_normalized",
        },
        "dataset_mix_info": {
            "joint_train": dataset_mix_info(joint_train_dataset),
            "joint_val": dataset_mix_info(joint_val_dataset),
            "point_warmup_train": dataset_mix_info(point_train_dataset),
            "kubric_warmup_train": dataset_mix_info(kubric_train_dataset),
        },
        "train_samples": len(joint_train_dataset),
        "val_samples": len(joint_val_dataset),
        "phase_schedule": phase_schedule,
        ...
        "protocol_lock": {
            ...
            "tapvid_main_eval": True,
            "tapvid3d_limited_eval": True,
            "forbidden": {
                "stage2_semantics": False,
                "wan": False,
                "motioncrafter_vae": False,
                "dynamic_replica": False,
                "source_conditioning_new_variant": False,
                "new_data": False,
            },
        },
    }
    ...
    print(f"[fix2][{args.run_name}] wrote summary: {summary_path}")
    print(f"[fix2][{args.run_name}] checkpoints: {ckpt_dir}")
    return 0
